...
def _register_base(reg: BlockRegistry, v: StoneType) -> None:
    reg.register(BlockDefinition(block_id=block_id(v), display_name=str(v.display), textures=v.textures, kind=str(v.kind), family="block", is_full_cube=bool(v.is_full_cube), is_solid=True, tags=_STONE_LIKE_TAGS))

def _register_slab(reg: BlockRegistry, v: StoneType) -> None:
    bid = slab_id(v)
    if bid is None:
        return
    reg.register(BlockDefinition(block_id=bid, display_name=_variant_display(str(v.display), "Slab"), textures=v.textures, kind="slab", family="slab", is_full_cube=False, is_solid=True, tags=_STONE_LIKE_TAGS))

def _register_stairs(reg: BlockRegistry, v: StoneType) -> None:
    bid = stairs_id(v)
    if bid is None:
        return
    reg.register(BlockDefinition(block_id=bid, display_name=_variant_display(str(v.display), "Stairs"), textures=v.textures, kind="stairs", family="stairs", is_full_cube=False, is_solid=True, tags=_STONE_LIKE_TAGS))

def _register_wall(reg: BlockRegistry, v: StoneType) -> None:
    bid = wall_id(v)
    if bid is None:
        return
    reg.register(BlockDefinition(block_id=bid, display_name=_variant_display(str(v.display), "Wall"), textures=v.textures, kind="wall", family="wall", is_full_cube=False, is_solid=True, tags=_STONE_LIKE_TAGS))

def _register_fence(reg: BlockRegistry, v: StoneType) -> None:
    bid = fence_id(v)
    if bid is None:
        return
    reg.register(BlockDefinition(block_id=bid, display_name=_variant_display(str(v.display), "Fence"), textures=v.textures, kind="fence", family="fence", is_full_cube=False, is_solid=True, tags=_STONE_LIKE_TAGS))
