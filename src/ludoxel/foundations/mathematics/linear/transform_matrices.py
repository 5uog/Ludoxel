...
def translate_matrix(x: float, y: float, z: float) -> np.ndarray:
  """
  三次元平行移動を 4×4 homogeneous transform として表す。
  返値 shape は `(4, 4)`、dtype は `np.float32` であり、移動量 `(x, y, z)` は列 3 の上三成分に格納される。
  """
...
def scale_matrix(x: float, y: float, z: float) -> np.ndarray:
  """
  各軸の独立 scale を 4×4 homogeneous transform として表す。
...
def rotate_x_rad_matrix(rad: float) -> np.ndarray:
  """
  x 軸まわりの回転角 `rad` を radian 単位で受け取り、4×4 回転行列を返す。
...
def rotate_y_rad_matrix(rad: float) -> np.ndarray:
  """
  y 軸まわりの回転角 `rad` を radian 単位で受け取り、4×4 回転行列を返す。
...
def rotate_z_rad_matrix(rad: float) -> np.ndarray:
  """
  z 軸まわりの回転角 `rad` を radian 単位で受け取り、4×4 回転行列を返す。
...
def rotate_z_deg_matrix(deg: float) -> np.ndarray:
  """
  degree 単位の z 軸回転角を radian へ変換して回転行列を生成する。
  visual animation が保持する degree 値を renderer へ渡す前の 4×4 回転行列へ変換する。
  """
