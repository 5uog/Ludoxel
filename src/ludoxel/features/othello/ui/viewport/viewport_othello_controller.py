...
from ludoxel.features.othello.domain.book.book_opening_book import export_opening_book_file, import_opening_book_file, opening_book_summary
from ludoxel.features.othello.domain.game.game_board import OTHELLO_BOARD_SURFACE_Y, raycast_board_square, square_center, square_index_to_name
from ludoxel.features.othello.domain.game.game_rules import counts_for_board
...
from ludoxel.shared.audio import PLAYER_EVENT_OTHELLO_FLIP, PLAYER_EVENT_OTHELLO_PLACE
...
def import_book(viewport: "RendererViewportWidget") -> None:
  ...
  if not str(selected_path).strip():
    return
  try:
    summary = import_opening_book_file(selected_path, project_root=viewport._data_root)
...
def export_book(viewport: "RendererViewportWidget") -> None:
  ...
  if not str(selected_path).strip():
    return
  try:
    export_path = export_opening_book_file(selected_path, project_root=viewport._data_root)
  except Exception as exc:
...
def maybe_request_ai(viewport: "RendererViewportWidget") -> None:
  ...
  project_root = str(viewport._data_root)
...
def maybe_request_analysis(viewport: "RendererViewportWidget") -> None:
  ...
  viewport._othello_ai.request_analysis(
    generation=int(state.match_generation),
    board=tuple(state.board),
