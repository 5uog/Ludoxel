...
def sync_hud_text(viewport: "GLViewportWidget") -> None:
    ...
    title = title_text(viewport, black_count=int(black_count), white_count=int(white_count))
    signature = (str(turn_text), int(black_count), int(white_count), str(difficulty), str(player_color), str(ai_color), str(black_clock), str(white_clock), str(state.message), str(title))
    if viewport._othello_hud_signature == signature:
        return
    viewport._othello_hud_signature = signature

    viewport._othello_hud.set_text("\n".join([str(turn_text), f"Black {int(black_count)}  White {int(white_count)}", f"AI {difficulty}  You {player_color}  AI {ai_color}", f"Black clock: {black_clock}", f"White clock: {white_clock}", str(state.message)]))
    viewport._othello_hud.set_title_text(title)
