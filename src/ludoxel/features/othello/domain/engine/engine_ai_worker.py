...
from ludoxel.features.othello.domain.engine.engine_ai import InsaneSearchCache, analyze_position, choose_ai_move
...
def _compute_book_learning(
  depth: int, per_move_error: float, cumulative_error: float, leaf_error: float, project_root: str, hash_level: int, sacrifice_level: int, progress_queue, cancel_event
) -> BookLearningResult:
  ...
  def request_book_learning(self, *, settings: OthelloSettings, project_root: str = "") -> None:
    ...
    try:
      future = executor.submit(
        ...
        int(normalized_settings.book_learning_depth),
        float(normalized_settings.book_per_move_error),
        float(normalized_settings.book_cumulative_error),
        float(normalized_settings.book_leaf_error),
        normalized_project_root,
        int(normalized_settings.hash_level),
        int(normalized_settings.sacrifice_level),
        self._book_learning_progress_queue,
