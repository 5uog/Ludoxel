...
from ludoxel.features.othello.domain.book.book_opening_book import OpeningBook, load_opening_book, normalize_project_root
...
def _adjacent_bits(bits: int) -> int:
  src = int(bits)
  return int(
    (_shift_east(src) | _shift_west(src) | _shift_south(src) | _shift_north(src) | _shift_south_east(src) | _shift_south_west(src) | _shift_north_east(src) | _shift_north_west(src)) & _FULL_MASK
  )
