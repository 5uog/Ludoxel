...
from ludoxel.features.othello.domain.engine.engine_evaluation_profile import POSITION_WEIGHTS, evaluation_weights
from ludoxel.features.othello.domain.engine.engine_insane_engine import InsaneSearchCache, analyze_insane_position, opening_book_moves
from ludoxel.features.othello.domain.game.game_rules import BOARD_SIZE, apply_move, counts_for_board, find_legal_moves, other_side
from ludoxel.features.othello.domain.game.game_types import (
  ...
  *,
  ...
  for ply_index in range(max(0, int(max_plies))):
    ...
    move_index = choose_ai_move(
      ...
      current_side,
      difficulty,
      random_seed=int(random_seed + ply_index * 17),
      project_root=project_root,
      strong_time_budget_s=float(strong_time_budget_s),
      insane_time_budget_s=float(insane_time_budget_s),
      match_generation=int(match_generation),
      insane_cache=insane_cache,
...
def analyze_position(
  ...
  *,
  random_seed: int = 0,
  project_root=None,
  strong_time_budget_s: float = 1.5,
  insane_time_budget_s: float = 4.0,
  match_generation: int = 0,
  insane_cache: InsaneSearchCache | None = None,
  sacrifice_level: int = DEFAULT_OTHELLO_SACRIFICE_LEVEL,
  hash_level: int = 2,
  include_forecast_line: bool = True,
) -> OthelloAnalysis:
  materialized = coerce_board(board)
  ...
  def build_forecast(max_plies: int) -> tuple[int, ...]:
    ...
    return _forecast_line(
      ...
      match_generation=int(match_generation),
      insane_cache=insane_cache,
      strong_time_budget_s=float(forecast_strong_budget_s),
      insane_time_budget_s=float(forecast_insane_budget_s),
  ...
  if mode == OTHELLO_DIFFICULTY_INSANE:
    ...
    return OthelloAnalysis(
      side_to_move=int(ai_side),
      best_move_index=analysis.best_move_index,
      best_line=build_forecast(8),
  ...
  return OthelloAnalysis(
    side_to_move=int(ai_side),
    best_move_index=best_result.move_index,
    best_line=build_forecast(6),
    score=float(best_result.score if math.isfinite(best_result.score) else 0.0),
    solved=False,
