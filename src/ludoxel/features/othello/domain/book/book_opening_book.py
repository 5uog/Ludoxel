...
from ludoxel.features.othello.domain.game.game_rules import apply_move, create_initial_board, find_legal_moves
from ludoxel.features.othello.domain.game.game_types import BOARD_CELL_COUNT, SIDE_BLACK, coerce_board, encode_board, normalize_side, other_side
from ludoxel.shared.shared_project_paths import default_project_root, default_runtime_data_root, runtime_cache_root, runtime_state_root

_BOARD_SIZE = 8


def _default_project_root() -> Path:
  """I define P_0 as the app-managed data root inferred from the source root. I use this anchor whenever the caller omits an explicit data root, because user book persistence must not be written into the package tree."""
...
def normalize_project_root(project_root: str | Path | None = None) -> Path:
  """I define N_P(x) as the total projection from an optional path-like input onto an absolute app-managed data root. I retain the public name for compatibility with existing engine cache callers."""
...
def user_opening_book_file_path(project_root: str | Path | None = None) -> Path:
  """I define P_u(root) = root/state/othello_opening_book.json. This location is the mutable user extension of the bundled book and therefore survives application restarts and interrupted learning sessions without writing into repository configs/."""
  return runtime_state_root(normalize_project_root(project_root)) / "othello_opening_book.json"


def _compiled_opening_book_cache_file_path(project_root: str | Path | None = None) -> Path:
  """I define P_c(root) = root/cache/othello_opening_book_cache.json. I use this cache artifact to persist the compiled position-indexed map without mixing cache data into user-authored state."""
  return runtime_cache_root(normalize_project_root(project_root)) / "othello_opening_book_cache.json"
...
def _load_user_opening_book_lines_cached(project_root_key: str) -> tuple[tuple[int, ...], ...]:
  ...
  if not verify_runtime_file(Path(project_root_key), "state/othello_opening_book.json"):
    return ()
...
def save_user_opening_book_lines(lines: tuple[tuple[int, ...], ...] | list[tuple[int, ...]] | list[list[int]], project_root: str | Path | None = None) -> tuple[tuple[int, ...], ...]:
  ...
  update_runtime_integrity_manifest(normalize_project_root(project_root), ("state/othello_opening_book.json",))
