...
def normalize_learning_mode(value: object) -> str:
  ...
  入力は任意 object を許容し、文字列化と前後空白除去と小文字化の後に LEARNING_MODES と明示一致した場合だけその値を採用し、欠落値・未知値は off へ退避する。off へ寄せるのは、未知値が誤って記録や policy 使用を有効化しない安全側の既定を保つためである。Train From Player Data と Train In Sandbox は値として保持できるが、通常 play 中に有効化してよい mode は ACTIVE_LEARNING_MODES に限られ、その制約は runtime 側が適用する。
  """
...
def is_active_learning_mode(mode: object) -> bool:
  ...
  正規化後の mode が off、observe_only、use_learned_policy の何れかである場合に真を返す。train 系 mode は UI 上で選択できても通常 play 中に重い学習を起動しないため、本述語は偽を返す。
  """
...
def _normalize_flag_map(value: object, *, keys: tuple[str, ...], default: bool) -> dict[str, bool]:
  ...
  返値は keys が定める key だけを持ち、保存値に存在する entry は coerce_bool で復元し、欠落 key は default を用いる。保存値に含まれる未知 key は捨て、形式変化や不正値が flag 集合へ混入しないようにする。
  """
...
class PersistedAiLearningSettings:
  ...
  learning_mode は Learning Mode の五値、capture_flags は demonstration 記録種別(RECORD_KINDS)ごとの取得有無、skill_flags は技能カテゴリごとの学習・評価対象有無、selected_policy_kind と selected_policy_id は本番使用する policy の種別と識別子、dataset_id は記録の蓄積先を区別する論理識別子である。capture_flags は既定で全種別偽(記録しない)、skill_flags は既定で全技能真とし、observe only 有効化と policy 使用の有無は learning_mode から導出する。
  ...
  def normalized(self) -> "PersistedAiLearningSettings":
    ...
    learning_mode は五値へ、selected_policy_kind は policy 種別四値へ正規化し、capture_flags は RECORD_KINDS に対し既定偽、skill_flags は技能 id 集合に対し既定真で正規化する。dataset_id は前後空白を除去し、空の場合は "default" を用いる。これにより UI と persistence と runtime が同一の値域と key 集合を共有する。
    """
  ...
  def recording_enabled(self) -> bool:
    ...
    Learning Mode が observe_only の場合に真を返す。off では記録せず、use_learned_policy と train 系は本基盤では記録の主目的とせず偽を返す。記録の種別別取捨は capture_flags が別途決定する。
    """
  ...
  def captured_kinds(self) -> tuple[str, ...]:
    ...
    記録が無効(recording_enabled が偽)な場合は空 tuple を返し、有効な場合は capture_flags が真の種別だけを返す。recorder の設定はこの列を用いる。
    """
  ...
  def to_dict(self) -> dict[str, Any]:
    ...
    返値は learning_mode、capture_flags、skill_flags、selected_policy_kind、selected_policy_id、dataset_id、及び導出値 observe_only を含む。observe_only は recording_enabled に等しい導出値であり、保存側の可読性のために併記するが、復元時の真値源は learning_mode である。
    """
  ...
  def from_dict(data: object) -> "PersistedAiLearningSettings":
    """
    mapping から設定を復元する。
    入力が dict でない場合は既定設定を返す。learning_mode、selected_policy_kind、selected_policy_id、dataset_id は文字列射影で読み取り、capture_flags と skill_flags は dict 以外を空 mapping として扱う。復元値は normalized を通して値域と key 集合へ収め、observe_only などの導出 key は無視する。
    """
...
class PersistedAiLearningState:
  """
  AI 学習基盤の保存状態一式を表す。
  settings は編集可能設定、dataset_summary は蓄積済み記録の規模要約(DatasetSummary.to_dict 形式)、last_training_summary は直近の学習結果要約、last_evaluation_summary は直近の評価結果要約、policy_version は本番使用 policy の内容版である。要約類は形式の自由度を保つため mapping として保持し、本 dataclass はそれらを直列化・復元する責務だけを持つ。
  ...
  def default() -> "PersistedAiLearningState":
    """
    新規環境向けの既定学習状態を返す。
    既定では Learning Mode が off、記録種別が全偽、技能が全真、policy は組み込み deterministic、各要約は空 mapping、policy_version は 0 である。off 既定により、設定 file が存在しない環境では記録も学習 policy 使用も発生しない。
    """
  ...
  def to_dict(self) -> dict[str, Any]:
    ...
    返値は schema_version、settings、dataset_summary、last_training_summary、last_evaluation_summary、policy_version を含む。要約類は浅い複製として埋め込み、復元と保存が同一形式を共有する。
    """
    return {
  ...
  def from_dict(data: object) -> "PersistedAiLearningState":
    """
    mapping から保存状態を復元する。
    入力が dict でない場合は既定状態を返す。settings は PersistedAiLearningSettings.from_dict で復元し、要約類は dict 以外を空 mapping として扱い、policy_version は数値化に失敗した場合 0 へ退避する。schema_version は数値化できない場合に現行版へ退避する。
    """
    if not isinstance(data, dict):
