...
# FILE: src/ludoxel/application/context/play_space_context.py
...
class PlaySpaceContext:
    ...
    def create_default(seed: int = 0) -> "PlaySpaceContext":
        ...
        my_world = create_my_world_session(
            seed=int(seed),
            block_registry=registry,
        )
        othello = create_othello_session(
            seed=int(seed),
            block_registry=registry,
        )

        return PlaySpaceContext(
            my_world=my_world,
            othello=othello,
            active_space_id=PLAY_SPACE_MY_WORLD,
        )
