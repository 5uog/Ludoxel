...
class AiLearningRuntime:
  """
  保存済み AI Learning 設定を live session の学習調整器へ接続し、
  demonstration の遅延 flush を管理する application 層 runtime である。
  本 runtime は user data root を所有する AiLearningStore と、
  user 学習 policy を user data root から解決する PolicyRegistry を保持する。
  session 開始時と Learning tab での設定変更後に configure_session を呼ぶことで、
  Learning Mode・記録対象種別・選択 policy を session の LearningCoordinator へ反映する。
  Use Learned Policy では選択 policy を registry 経由で解決し、
  評価未通過・破損・互換不一致の policy は registry が
  組み込み deterministic へ退避させるため、live AI は安全な決定へ fallback する。
  記録の永続化は tick の flush interval(既定 2 秒)に基づく遅延書き込みで行い、
  毎 frame の同期 I/O を避けて frame loop を止めない。
  shutdown 前には flush で未完了記録を user data root へ確実に書き出す。
  ...
  def __post_init__(self) -> None:
    ...
    registry は user 学習 policy の読み込みに本 runtime の loader を用い、
    組み込み deterministic を最終 fallback として保持する。
    """
  ...
  def _load_user_policy(self, policy_id: str) -> Policy | None:
    ...
    保存 mapping が無い又は復元不能の場合は None を返し、
    registry は組み込み deterministic へ退避する。
    """
  ...
  def configure_session(self, session) -> None:
    ...
    通常 play で有効化してよい mode は off / observe_only / use_learned_policy に限り、
    train 系 mode が保存されていても live では off として扱う。
    Use Learned Policy では選択 policy を registry で解決して渡し、
    評価未通過・破損 policy は registry の fallback により組み込み deterministic となる。
    記録対象種別と dataset 識別子も設定から取得し、後続 flush の宛先に用いる。
    """
  ...
  def tick(self, session, dt: float) -> int:
    ...
    経過時間が flush_interval_s 未満の間は何もせず 0 を返す。
    interval 到達時に flush 待ち記録があれば dataset へ追記し、書き出し件数を返す。
    毎 frame の同期 I/O を避けるため、本 method は frame loop から呼んでも interval 間隔でしか書き込みを行わない。
    """
  ...
  def flush(self, session) -> int:
    ...
    flush 待ち記録が無ければ 0 を返す。
    shutdown 前や明示的な保存契機で呼び、記録の損失を防ぐ。
    """
    if int(session.learning_pending()) <= 0:
      return 0
    return int(session.flush_learning(self._store.dataset_writer(self._dataset_id)))
