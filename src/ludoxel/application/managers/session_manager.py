...
# FILE: src/ludoxel/application/managers/session_manager.py
...
class SessionManager:
    ...
    def create_default(seed: int = 0) -> "SessionManager":
        ...
        player = PlayerEntity(
            position=Vec3(
                float(settings.spawn_x),
                float(settings.spawn_y),
                float(settings.spawn_z),
            ),
    ...
    def _update_creative_flight_toggle(self, *, creative_mode: bool, jump_pressed: bool) -> None:
        ...
        if bool(self.player.flying):
            self.player.velocity = Vec3(
                float(self.player.velocity.x),
                0.0,
                float(self.player.velocity.z),
            )
        ...
        self.player.velocity = Vec3(
            float(self.player.velocity.x),
            min(0.0, float(self.player.velocity.y)),
            float(self.player.velocity.z),
        )
    ...
    def _update_player_walk_phase(self, dt: float) -> bool:
        ...
        if bool(player.flying) or (not bool(player.on_ground)) or speed < float(_PLAYER_FOOTSTEP_MIN_SPEED):
            return False
        return int(math.floor(previous_total / math.pi)) != int(
            math.floor(float(self._player_walk_phase_total_rad) / math.pi)
        )

    def _support_contact(self) -> tuple[str | None, tuple[int, int, int] | None]:
        contact = support_block_beneath(
            self.player,
            self.world,
            block_registry=self.block_registry,
    ...
    def step(
        ...
        self._update_creative_flight_toggle(
        ...
        *,
        ...
        if float(player.crouch_eye_drop) > 1e-9:
            crouch_amount = float(
                max(
                    0.0,
                    min(1.0, float(player.crouch_eye_offset) / float(player.crouch_eye_drop)),
                )
            )

        walk_speed = max(1e-6, float(self.settings.movement.walk_speed))
        speed_ratio = clampf(
            float(speed) / float(walk_speed),
            0.0,
            float(_PLAYER_WALK_MAX_SWING_SCALE),
        )
        ...
        camera = CameraDTO(
            ...
            shake_tz=float(cam_shake_tz),
            shake_yaw_deg=float(cam_shake_yaw_deg),
    ...
    def break_block(
        ...
        *,
        ...
        return self.interaction.break_block(
            reach=float(reach),
            origin=origin,
            direction=direction,
        )
