...
def normalize_cloud_speed_range(min_speed: object, max_speed: object) -> tuple[float, float]:
  ...
  両端は 0 以上 4 以下へ clamp し、入力順が逆の場合は交換するため、persistence、UI、renderer は常に `min_speed <= max_speed` を共有する。
  """
...
def normalize_cloud_height_settings(
  ...
  高度は 28 以上 250 以下、確率は 0 以上 100 以下へ clamp し、逆転した区間を交換した後で優先区間を生成範囲内へ射影する。
  """
