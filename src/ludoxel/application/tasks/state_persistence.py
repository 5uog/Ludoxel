...
from ...features.othello.domain.game.board import (
...
from ..pipelines.runtime_state_pipeline import (
...
def _load_player_into_session(
    *,
    ...
    runtime_player.crouch_eye_offset = float(
        max(0.0, min(float(runtime_player.crouch_eye_drop), float(player.crouch_eye_offset)))
    )
...
def _maybe_replace_world(session: SessionManager, persisted_world: PersistedWorld) -> None:
    ...
    session.world.replace_all(
        blocks={key: str(value) for (key, value) in persisted_world.blocks.items()},
...
def _lift_player_above_othello_board_if_needed(session: SessionManager) -> None:
    ...
    player.velocity = Vec3(
        ...
        max(0.0, float(player.velocity.y)),
...
def _persisted_player_from_session(session: SessionManager, *, allow_flying: bool) -> PersistedPlayer:
    ...
    return PersistedPlayer(
        ...
        crouch_eye_offset=float(
            max(0.0, min(float(player.crouch_eye_drop), float(player.crouch_eye_offset)))
        ),
    )
...
def _persisted_world_from_session(session: SessionManager) -> PersistedWorld:
    ...
    return PersistedWorld(
        ...
        blocks={key: str(value) for (key, value) in snapshot.items()},
    )
...
def apply_persisted_state_if_present(
    *,
    ...
    renderer,
) -> tuple[RuntimePreferences, OthelloGameState]:
    ...
    if state is not None:
        ...
        _load_player_into_session(
            session=sessions.my_world,
            player=state.my_world.player,
            allow_flying=bool(runtime.creative_mode),
        )
        _maybe_replace_world(sessions.my_world, state.my_world.world)

        _load_player_into_session(
            session=sessions.othello,
            player=state.othello_space.player,
            allow_flying=False,
        )
    ...
    **overrides,
    ...
    state = AppState(
        ...
        my_world=PersistedPlaySpace(
            player=_persisted_player_from_session(
                sessions.my_world,
                allow_flying=bool(state_runtime.creative_mode),
            ),
