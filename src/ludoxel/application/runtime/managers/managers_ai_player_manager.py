...
from ludoxel.application.runtime.managers.managers_player_combat import (
  MELEE_ATTACK_DAMAGE,
  MELEE_ATTACK_REACH_BLOCKS,
  MELEE_DAMAGE_COOLDOWN_S,
  apply_melee_damage,
  apply_void_damage,
  attack_sprinting,
  pick_player_target,
)
from ludoxel.application.runtime.managers.managers_player_kinematics import PlayerMotionState, PlayerStepInput, advance_runtime_player, build_player_model_snapshot, fall_damage_amount
...
from ludoxel.application.runtime.tasks.tasks_ai_route_planner import AiRoutePlanRequest, AiRoutePlanResult, AiRoutePlanStep
from ludoxel.application.runtime.tasks.tasks_ai_route_worker import AiRouteWorker
from ludoxel.shared.blocks.models.models_api import collision_aabbs_for_block, has_full_top_support_for_block
from ludoxel.shared.blocks.registry.registry_block_registry import BlockRegistry
from ludoxel.shared.blocks.structure.structure_cardinal import cardinal_from_xz, facing_vec_xz
from ludoxel.shared.math.math_scalars import clampf
from ludoxel.shared.math.math_vec3 import Vec3
from ludoxel.shared.math.math_view_angles import yaw_pitch_deg_from_forward
from ludoxel.shared.rendering.rendering_first_person_motion import FirstPersonMotionSample
from ludoxel.shared.rendering.rendering_player_render_state import PlayerRenderState
from ludoxel.shared.rendering.rendering_player_render_state_composer import compose_player_render_state_from_parts
from ludoxel.shared.systems.systems_block_pick import BlockPick
from ludoxel.shared.systems.systems_collision_system import _any_intersection, support_block_beneath
from ludoxel.shared.systems.systems_interaction_service import InteractionService
...
def _combat_control(*, actor: _AiPlayerRuntime, target: Vec3, dt: float, jump_pressed: bool = False) -> PlayerStepInput:
  ...
  strafe = 0.0
  if (
    float(actor.combat_strafe_timer_s) > 1e-6
...
def _parkour_control(
  *, player: PlayerEntity, target: Vec3, dt: float, sprint: bool, auto_jump_enabled: bool, jump_pressed: bool = False, crouch: bool = False, commit_forward: bool = False
) -> PlayerStepInput:
  delta = target - player.eye_pos()
  horizontal_length = math.hypot(float(delta.x), float(delta.z))
  if horizontal_length <= 1e-6 and abs(float(delta.y)) <= 1e-6:
  ...
  move_f, move_s = _movement_inputs_toward_target(
    player=player, target=target, yaw_deg=float(player.yaw_deg) + float(yaw_delta_deg), remaining_yaw_error_deg=float(remaining_yaw_error_deg), slowdown_radius=0.28
  )
...
class AiPlayerManager:
  ...
  def render_states(self) -> tuple[PlayerRenderState, ...]:
    ...
    for actor in self._actors.values():
      ...
      states.append(
        compose_player_render_state_from_parts(player_model=player_model, motion=motion, block_registry=self.block_registry, arm_rotation_limit_min_deg=-180.0, arm_rotation_limit_max_deg=180.0)
      )
    return tuple(states)
