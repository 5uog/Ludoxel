...
from ludoxel.application.runtime.state.state_runtime_preferences import RuntimePreferences, coerce_runtime_preferences
from ludoxel.features.othello.domain.game.game_board import OTHELLO_BOARD_SURFACE_Y, ensure_othello_board_layout, is_othello_board_footprint
from ludoxel.features.othello.domain.game.game_types import OthelloGameState
from ludoxel.shared.blocks.models.models_api import collision_aabbs_for_block
from ludoxel.shared.blocks.structure.structure_structural_rules import is_fence_gate
from ludoxel.shared.math.math_vec3 import Vec3
from ludoxel.shared.systems.systems_gravity_system import GRAVITY_AFFECTED_TAG
from ludoxel.shared.world.world_play_space import normalize_play_space_id

from ....shared.blocks.state.state_codec import parse_state
from ....shared.blocks.state.state_values import prop_as_bool
from ..persistence import AppState, AppStateStore, PersistedAiPlayer, PersistedOthelloSpace, PersistedPlayer, PersistedPlaySpace, PersistedWorld
...
def _persisted_player_from_session(session: SessionManager, *, allow_flying: bool) -> PersistedPlayer:
  ...
  return PersistedPlayer(
    pos_x=float(player.position.x),
    pos_y=float(player.position.y),
    pos_z=float(player.position.z),
    vel_x=float(player.velocity.x),
    vel_y=float(player.velocity.y),
    vel_z=float(player.velocity.z),
    yaw_deg=float(player.yaw_deg),
    pitch_deg=float(player.pitch_deg),
    on_ground=bool(player.on_ground),
    flying=bool(player.flying and allow_flying),
    auto_jump_cooldown_s=float(max(0.0, float(player.auto_jump_cooldown_s))),
    crouch_eye_offset=float(max(0.0, min(float(player.crouch_eye_drop), float(player.crouch_eye_offset)))),
