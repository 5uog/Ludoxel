...
from ludoxel.shared.rendering.rendering_player_render_state_composer import compose_player_render_state

from ....audio import PLAYER_EVENT_DAMAGE_HIT, PLAYER_EVENT_LAND, PLAYER_EVENT_STEP
...
class ViewportRenderLoopMixin:
  ...
  def _othello_state_changed(state_before, state_after) -> bool:
    return bool(
      state_before.status != state_after.status
      or state_before.board != state_after.board
      or state_before.current_turn != state_after.current_turn
      or state_before.legal_moves != state_after.legal_moves
      or state_before.thinking != state_after.thinking
      or state_before.last_move_index != state_after.last_move_index
      or state_before.animations != state_after.animations
      or state_before.message != state_after.message
      or state_before.winner != state_after.winner
    )
  ...
  def _on_step(self: "GLViewportWidget", dt: float) -> None:
    ...
    step_result = self._session.step(
      ...
      move_f=frame.move_f,
      move_s=frame.move_s,
      jump_held=bool(frame.jump_held),
      jump_pressed=bool(frame.jump_pressed),
      sprint=bool(sprint),
      crouch=bool(frame.crouch),
      mdx=float(mouse_delta.dx),
      mdy=float(mouse_delta.dy),
