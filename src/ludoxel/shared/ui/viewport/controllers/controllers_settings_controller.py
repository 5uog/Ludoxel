...
from ludoxel.application.runtime.pipelines.pipelines_runtime_state_pipeline import apply_runtime_to_renderer as apply_runtime_to_renderer_state, sync_runtime_sun_from_renderer
from ludoxel.application.runtime.runtime_keybinds import KeybindSettings
from ludoxel.application.runtime.runtime_player_name import normalize_player_name, resolve_session_player_name
from ludoxel.application.runtime.state.state_audio_preferences import AudioPreferences
from ludoxel.application.runtime.state.state_camera_perspective import normalize_camera_perspective
...
def sync_cloud_motion_pause(viewport: "GLViewportWidget") -> None:
  pause_motion = (
    ...
    or bool(viewport._overlays.dead())
    or bool(viewport._overlays.inventory_open())
    or bool(viewport._overlays.othello_settings_open())
    or bool(getattr(viewport, "_ai_settings_overlay_open", False))
    or bool(getattr(viewport, "_transient_modal_active", lambda: False)())
    or (not bool(viewport._application_active))
  )
...
def sync_settings_values(viewport: "GLViewportWidget") -> None:
  ...
  viewport._settings.sync_values(
    ...
    crosshair_mode=str(viewport._state.crosshair_mode),
    crosshair_pixels=tuple(viewport._state.crosshair_pixels),
    camera_perspective=str(viewport._state.camera_perspective),
    ...
    player_name=str(viewport._state.player_name),
    resolved_player_name=str(viewport._state.resolved_player_name),
    block_break_repeat_interval_s=float(viewport._state.block_break_repeat_interval_s),
    block_place_repeat_interval_s=float(viewport._state.block_place_repeat_interval_s),
    block_interact_repeat_interval_s=float(viewport._state.block_interact_repeat_interval_s),
