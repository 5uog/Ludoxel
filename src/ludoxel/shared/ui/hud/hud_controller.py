...
from ...diagnostics import GpuUtilizationSampler, ProcessMemorySnapshot, SystemInfo, read_process_memory, read_system_info
...
class HudController:
  ...
  def _build_left_text(
    ...
    *,
    ...
    lines.append(
      f"World Wireframe: {int(bool(world_wire))} | Shadow: {int(bool(shadow_ok))} | Size: {int(shadow_size)} | DBG: {int(bool(debug_shadow))} | Sun: {float(sun_az_deg):.0f}/{float(sun_el_deg):.0f}\n"
    )
