...
_STONE_VARIANT_RECIPES: tuple[CatalogVariantRecipe, ...] = (
    CatalogVariantRecipe(
        variant_id=lambda stone: block_id(stone),
        display_name=lambda stone: str(stone.display),
        kind=lambda stone: str(stone.kind),
        family="block",
        is_full_cube=lambda stone: bool(stone.is_full_cube),
    ),
    CatalogVariantRecipe(
        variant_id=lambda stone: slab_id(stone),
        display_name=lambda stone: _variant_display(str(stone.display), "Slab"),
        kind="slab",
        family="slab",
        is_full_cube=False,
    ),
    CatalogVariantRecipe(
        variant_id=lambda stone: stairs_id(stone),
        display_name=lambda stone: _variant_display(str(stone.display), "Stairs"),
        kind="stairs",
        family="stairs",
        is_full_cube=False,
    ),
    CatalogVariantRecipe(
        variant_id=lambda stone: wall_id(stone),
        display_name=lambda stone: _variant_display(str(stone.display), "Wall"),
        kind="wall",
        family="wall",
        is_full_cube=False,
    ),
    CatalogVariantRecipe(
        variant_id=lambda stone: fence_id(stone),
        display_name=lambda stone: _variant_display(str(stone.display), "Fence"),
        kind="fence",
        family="fence",
        is_full_cube=False,
    ),
...
def register_stones(reg: BlockRegistry) -> None:
    for v in _all_stones():
        register_catalog_variants(
            ...
            v,
            textures=v.textures,
            tags=_STONE_LIKE_TAGS,
            recipes=_STONE_VARIANT_RECIPES,
            sound_group=lambda stone: getattr(stone, "sound_group", "stone"),
        )
