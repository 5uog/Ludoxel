...
from ludoxel.shared.rendering.backend.backend_renderer_metrics import BackendRendererFrameMetrics
from ludoxel.shared.rendering.backend.backend_renderer_state import DEFAULT_BACKEND_CLOUD_FLOW_DIRECTION, BackendRendererRuntimeState
...
class Renderer:
  def __init__(self, params: BackendRendererParams | None = None, *, canvas=None, preferred_backend: str | None = None) -> None:
    ...
    self._backend = create_backend_renderer(cfg=self._cfg, state=self._state, canvas=canvas, preferred_backend=preferred_backend)
  ...
  def frame_metrics(self) -> BackendRendererFrameMetrics:
