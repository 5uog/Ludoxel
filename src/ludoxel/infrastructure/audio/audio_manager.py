...
class AudioManager(QObject):
    ...
    def set_preferences(self, preferences: AudioPreferences) -> None:
        ...
        if self._ambient_output is not None:
            self._ambient_output.setVolume(
                float(self._preferences.volume_for(AUDIO_CATEGORY_AMBIENT))
            )
    ...
    def prime_effects(self) -> None:
        ...
        for pool_key, pool in self._pool_specs.items():
            ...
            for prepared in tuple(prepared_sources):
                self._ensure_effect_slots(
                    prepared,
                    desired_slots=int(prepared.desired_slots),
                    base_volume=float(base_volume),
                )

        self._effects_primed = True

    def cache_listener_pose(
        self,
        *,
        eye: Vec3,
        yaw_deg: float,
    ...
    def _ensure_prepared_sources(self, pool_key: str, pool: AudioSamplePool) -> list[_PreparedSource]:
        ...
        prepared = [
            _PreparedSource(
                ...
                desired_slots=int(desired_slots),
            )
    ...
    def _next_effect_slot(
        self,
        prepared: _PreparedSource,
        *,
        desired_slots: int,
        base_volume: float,
    ) -> _EffectVoiceSlot | None:
        self._ensure_effect_slots(
            prepared,
            desired_slots=int(desired_slots),
            base_volume=float(base_volume),
