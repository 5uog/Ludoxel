...
class WorldState:
  ...
  def _chunk_column_band(self, cx: int, cz: int) -> tuple[int, int] | None:
    ...
    if hi < BEDROCK_Y:
      ...
      neighbor_min = np.minimum.reduce((heights[0:CHUNK_SIZE, 1 : CHUNK_SIZE + 1], heights[2 : CHUNK_SIZE + 2, 1 : CHUNK_SIZE + 1], heights[1 : CHUNK_SIZE + 1, 0:CHUNK_SIZE], heights[1 : CHUNK_SIZE + 1, 2 : CHUNK_SIZE + 2]))
