...
_CHANNEL_HEIGHT_BASE: int = 1
_CHANNEL_RAVINE: int = 101
_CHANNEL_RAVINE_MASK: int = 102
_CHANNEL_RAVINE_DEPTH: int = 103
_CHANNEL_DEEPSLATE: int = 120
_CHANNEL_DIRT_DEPTH: int = 130
_CHANNEL_DIRT_MIX: int = 131
_CHANNEL_DIRT_PATCH: int = 132
_CHANNEL_STONE_A: int = 140
_CHANNEL_STONE_B: int = 141
_CHANNEL_ORE_GATE: int = 150
_CHANNEL_EMERALD: int = 151
_CHANNEL_ORE_COAL: int = 160
_CHANNEL_ORE_COPPER: int = 161
_CHANNEL_ORE_IRON: int = 162
_CHANNEL_ORE_GOLD: int = 163
_CHANNEL_ORE_REDSTONE: int = 164
_CHANNEL_ORE_LAPIS: int = 165
_CHANNEL_ORE_DIAMOND: int = 166
...
def hash_u64(seed: int, channel: int, a: int, b: int, c: int = 0) -> int:
  v = int(seed) & _U64_MASK
  v ^= (int(channel) * 0x9E3779B97F4A7C15) & _U64_MASK
...
def _lattice_value(seed: int, channel: int, a: int, b: int, c: int = 0) -> float:
  h = hash_u64(seed, channel, a, b, c)
...
def value_noise_2d(seed: int, channel: int, x: float, z: float, scale: float) -> float:
  ...
  xi = int(x0)
  zi = int(z0)
  n00 = _lattice_value(seed, channel, xi, zi)
  n10 = _lattice_value(seed, channel, xi + 1, zi)
  n01 = _lattice_value(seed, channel, xi, zi + 1)
  n11 = _lattice_value(seed, channel, xi + 1, zi + 1)
  nx0 = n00 + (n10 - n00) * sx
  nx1 = n01 + (n11 - n01) * sx
  return nx0 + (nx1 - nx0) * sz


def value_noise_3d(seed: int, channel: int, x: float, y: float, z: float, scale: float) -> float:
  fx = float(x) / float(scale)
  ...
  yi = int(y0)
  zi = int(z0)
  n000 = _lattice_value(seed, channel, xi, zi, yi)
  n100 = _lattice_value(seed, channel, xi + 1, zi, yi)
  n010 = _lattice_value(seed, channel, xi, zi + 1, yi)
  n110 = _lattice_value(seed, channel, xi + 1, zi + 1, yi)
  n001 = _lattice_value(seed, channel, xi, zi, yi + 1)
  n101 = _lattice_value(seed, channel, xi + 1, zi, yi + 1)
  n011 = _lattice_value(seed, channel, xi, zi + 1, yi + 1)
  n111 = _lattice_value(seed, channel, xi + 1, zi + 1, yi + 1)
...
def raw_surface_height(seed: int, x: int, z: int) -> float:
  total = _BASE_HEIGHT
  channel = _CHANNEL_HEIGHT_BASE
  for amplitude, wavelength in _HEIGHT_OCTAVES:
    total += amplitude * value_noise_2d(seed, channel, float(x), float(z), wavelength)
    channel += 1
  return total


def ravine_depth(seed: int, x: int, z: int) -> float:
  mask = value_noise_2d(seed, _CHANNEL_RAVINE_MASK, float(x), float(z), _RAVINE_MASK_SCALE)
  if mask <= _RAVINE_MASK_MIN:
    return 0.0
  ridge = value_noise_2d(seed, _CHANNEL_RAVINE, float(x), float(z), _RAVINE_SCALE)
  distance = abs(ridge)
  if distance >= _RAVINE_EDGE:
    return 0.0
  t = 1.0 - distance / _RAVINE_EDGE
  s = _smoothstep(t)
  depth_amp = 14.0 + 7.0 * value_noise_2d(seed, _CHANNEL_RAVINE_DEPTH, float(x), float(z), _RAVINE_DEPTH_SCALE)
  if depth_amp < 4.0:
    depth_amp = 4.0
  return s * depth_amp
...
def _stone_material(seed: int, x: int, y: int, z: int, deepslate: bool) -> int:
  ...
  v1 = value_noise_3d(seed, _CHANNEL_STONE_A, float(x), float(y), float(z), 34.0)
  if v1 > 0.52:
    return _MATERIAL_ANDESITE
  if v1 < -0.52:
    return _MATERIAL_GRANITE
  v2 = value_noise_3d(seed, _CHANNEL_STONE_B, float(x), float(y), float(z), 23.0)
  if v2 > 0.55:
...
def _ore_material(seed: int, x: int, y: int, z: int, deepslate: bool) -> int:
  gate = hash_u64(seed, _CHANNEL_ORE_GATE, x, z, y) % 1000
  ore = 0
  if gate < 350:
    if y <= -40 and value_noise_3d(seed, _CHANNEL_ORE_DIAMOND, float(x), float(y), float(z), 10.0) > 0.78:
      ore = _MATERIAL_DIAMOND_ORE
    elif -64 <= y <= -8 and value_noise_3d(seed, _CHANNEL_ORE_GOLD, float(x), float(y), float(z), 12.0) > 0.72:
      ore = _MATERIAL_GOLD_ORE
    elif -64 <= y <= -16 and value_noise_3d(seed, _CHANNEL_ORE_REDSTONE, float(x), float(y), float(z), 12.0) > 0.72:
      ore = _MATERIAL_REDSTONE_ORE
    elif -64 <= y <= -16 and value_noise_3d(seed, _CHANNEL_ORE_LAPIS, float(x), float(y), float(z), 12.0) > 0.74:
      ore = _MATERIAL_LAPIS_ORE
    elif y <= 20 and value_noise_3d(seed, _CHANNEL_ORE_IRON, float(x), float(y), float(z), 14.0) > 0.66:
      ore = _MATERIAL_IRON_ORE
    elif y >= -16 and value_noise_3d(seed, _CHANNEL_ORE_COPPER, float(x), float(y), float(z), 14.0) > 0.66:
      ore = _MATERIAL_COPPER_ORE
    elif value_noise_3d(seed, _CHANNEL_ORE_COAL, float(x), float(y), float(z), 16.0) > 0.62:
      ore = _MATERIAL_COAL_ORE
  if ore == 0 and y >= 0 and hash_u64(seed, _CHANNEL_EMERALD, x, z, y) % 4000 == 0:
...
def material_code(seed: int, version: int, mode: int, flat_ground_y: int, x: int, y: int, z: int, column_height: int | None = None) -> int:
  ...
  deepslate_boundary = -28.0 + 5.0 * value_noise_2d(seed, _CHANNEL_DEEPSLATE, float(x), float(z), 60.0)
  ...
  if not in_ravine:
    dirt_depth = 2 + int(hash_u64(seed, _CHANNEL_DIRT_DEPTH, x, z, 0) % 2)
    if depth <= dirt_depth:
      patch = value_noise_2d(seed, _CHANNEL_DIRT_PATCH, float(x), float(z), 24.0)
      roll = int(hash_u64(seed, _CHANNEL_DIRT_MIX, x, z, y) % 100)
      if patch > 0.45:
