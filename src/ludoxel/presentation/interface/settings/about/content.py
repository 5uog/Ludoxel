...
ABOUT_PROJECT_OVERVIEW_SECTIONS: tuple[AboutSection, ...] = (
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "The source tree is organized into four principal layers: `ludoxel.foundations`, `ludoxel.simulation`, `ludoxel.application`, and `ludoxel.presentation`. `foundations` contains project identity, root-location resolution, diagnostics, scalar coercion, numeric helpers, vector and matrix operations, view-angle calculations, AABB and ray mathematics, voxel DDA traversal, face constants, chunk-coordinate functions, and frustum clipping. It is the lowest layer because these facilities are used by both domain logic and presentation logic while carrying no Qt, renderer-backend, persistence-store, or play-space orchestration responsibilities."
      ),
      paragraph(
        "`simulation` owns domain state and rule execution. Its subpackages define worlds, generation fixtures, block definitions, block textures, block registries, block states, block-state codecs, block model geometry, structural connectivity, player entities, player movement, player damage, AI-player behavior, AI routing, AI combat, inventories, special items, collision, gravity, placement, picking, interaction, My World, Othello board rules, Othello engines, Othello matches, Othello books, Othello resources, and Othello control inventories. This layer is the owner of gameplay meaning: a block placement, legal Othello move, collision AABB, player health transition, or AI route decision is defined here rather than in a renderer pass or Qt widget."
      ),
      paragraph(
        "`application` owns orchestration and persisted application meaning. It contains bootstrap, runtime preferences, camera preferences, keybind preferences, crosshair schema, player-name normalization, player-skin kind, audio preferences, persistence schema, JSON file stores, integrity manifest handling, Othello book storage hooks, play-space context creation, session factories, session managers, runtime-state application, render-snapshot DTO construction, fixed-step runners, and save scheduling. This layer can assemble `simulation` and `foundations`, but its ordinary modules do not own Qt widgets, renderer buffers, shader programs, or audio playback implementations."
      ),
      paragraph(
        "`presentation` owns the desktop-facing implementation. It includes Qt windows, viewport widgets, input adapters, macOS cursor and keyboard guards, HUD widgets, route overlays, pause inventory and death overlays, skin preview, settings pages, About-page renderer and widgets, theme resources, audio playback catalogues, renderer contracts, OpenGL runtime, wgpu runtime, shader resources, texture atlases, visible-face construction, player visual state, Othello visual state, and render-loop lifecycle. Presentation can consume application and simulation state, but it must not redefine domain legality, persistence format, or package-root ownership."
      ),
      paragraph(
        "The dependency direction is architectural, not ornamental. `foundations` sits below the other layers; `simulation` may depend on foundations; `application` may depend on simulation and foundations; `presentation` may depend on application, simulation, and foundations. The unavoidable final connection from application startup to Qt shell is localized as a composition-root exception in `src/ludoxel/application/bootstrap/run.py`, where `ludoxel.presentation.interface.windows.main.run_app` is imported only after roots and storage hooks have been resolved. That exception should not be generalized into persistence, preference, store, schema, session, or simulation modules."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Application startup begins at `src/ludoxel/__main__.py` for module execution and at the console script declared in package metadata for installed execution. The module entry calls `multiprocessing.freeze_support()` before `run_app()` so that frozen desktop applications and multiprocessing-aware code paths do not enter uncontrolled child-process behavior. The imported `run_app` symbol is resolved by `ludoxel.application.__getattr__`, which imports `ludoxel.application.bootstrap` lazily. The lazy facade keeps package import cheap and prevents early presentation import before the actual startup phase."
      ),
      paragraph(
        "`ludoxel.application.bootstrap.run.run_app()` is the composition function. It computes the project root by calling `default_project_root(Path(__file__))`, computes the resource root by calling `default_resource_root(Path(__file__))`, computes the data root by calling `default_runtime_data_root(project_root)`, and only then evaluates the Python-version preference and presentation-shell import. Root calculation is centralized because the same application must work from a source tree, an editable install, and a PyInstaller bundle with a different resource layout."
      ),
      paragraph(
        "The bootstrap module contains a Python 3.14 preference path for non-frozen execution. `_ensure_python_314(project_root)` returns immediately when the application is frozen or already running under Python 3.14. Otherwise, `_preferred_python_314()` searches local installation candidates, and the bootstrap can re-execute `python -m ludoxel` under the preferred interpreter while preserving `PYTHONPATH` for the source root. This behavior is deliberately outside persistence and presentation because interpreter selection is an execution concern, not a widget concern."
      ),
      paragraph(
        "Othello book storage hooks are installed immediately before the presentation shell is imported. `install_othello_book_storage_hooks()` connects simulation-level opening-book functions to application-owned runtime paths, user-book files, compiled-cache files, JSON IO, and integrity manifest updates. This placement prevents the Othello simulation book module from importing application root policy directly while still allowing the application to provide persistent user book storage."
      ),
      paragraph(
        "The final startup transfer calls `ludoxel.presentation.interface.windows.main.run_app(project_root=project_root, resource_root=resource_root, data_root=data_root)`. The three roots are therefore already established before the Qt window, renderer widget, theme loader, font loader, audio manager, settings overlay, or viewport lifecycle code begins. This is important because all of those components need consistent access to immutable package resources and mutable runtime state."
      ),
      paragraph(
        "Runtime root policy has platform-specific branches. `LUDOXEL_DATA_ROOT` is an explicit override. Windows defaults to `%LOCALAPPDATA%/Ludoxel`; Darwin defaults to `~/Library/Application Support/Ludoxel`; other platforms follow XDG-style or home-directory fallback behavior. Runtime state and cache directories are produced underneath this data root. Repository-level `configs/` remains a previous-format migration source, not the normal v3.6 write target."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "The desktop shell is the user-facing composition of startup, state restoration, renderer initialization, input capture, modal surfaces, settings, and play-space switching. The main window restores persisted window geometry, applies theme resources, installs bundled fonts, loads icon and title-mark resources, constructs `GameScreen`, attaches the renderer viewport, and coordinates loading overlay state. Minimum dimensions such as `980 x 620` belong to the shell layer because they define the usable desktop surface for settings, gameplay, pause, Othello HUD, and live preview rather than domain simulation."
      ),
      paragraph(
        "`GameScreen` selects the platform viewport widget. Darwin uses `RendererViewportWidget`, which enters the renderer-contract path that can host the wgpu backend through rendercanvas. Non-Darwin platforms use `GLViewportWidget`, which enters the OpenGL-backed Qt surface path. This split is performed before resource loading and frame rendering because renderer initialization, canvas availability, OpenGL format requests, macOS cursor policy, and shader-resource expectations differ by platform."
      ),
      paragraph(
        "The desktop shell hosts two persistent spaces: `PLAY_SPACE_MY_WORLD = my_world` and `PLAY_SPACE_OTHELLO = othello`. `PlaySpaceContext` contains one `SessionManager` for each space and exposes `session_for()`, `active_session()`, and `set_active_space()`. `RuntimePreferences.current_space_id` and `AppState.current_space_id` persist the active space. Switching spaces is therefore not a temporary screen overlay; it is a state transition among persisted sessions that preserves player state, world state, AI actor state, inventory branch, Othello state, and renderer upload lifecycle."
      ),
      paragraph(
      ...
      paragraph(
        "The pause surface also hosts a live player preview. That preview uses the renderer API rather than duplicating skin UV logic in Qt painting code. The preview can rotate the player body, update head yaw and pitch, import a custom skin, reset to default Alex skin, and show a name tag when the same camera and HUD conditions that support in-world tags are active. This ties shell UI to renderer contract methods such as `set_player_skin_image()` and `render_player_preview_frame()`."
      ),
      paragraph(
        "Startup loading is also treated as shell state. The loading overlay begins with text such as `Preparing viewport...`, and completion is tied to renderer initialization and initially needed world upload residency. That prevents a false-ready state where the window exists but the renderer, chunk uploads, skin resources, or selection state are not ready. The shell can suspend input while modal surfaces are open and re-arm world upload and selection refresh after returning to gameplay."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Application persistence is divided into schema records and stores. `AppState` is the in-memory aggregate: it stores `current_space_id`, `settings`, `inventory`, `othello_settings`, `my_world`, and `othello_space`. It does not directly own JSON parsing, manifest verification, old-format migration, or renderer objects. The aggregate exists so the application can pass complete saved state through one object while the actual schema modules remain specialized: settings, inventory, player, world, AI player, play-space bundle, Othello-space bundle, and file envelopes."
      ),
      paragraph(
        "`PlayerStateFile` is the player-side envelope. Its current version is `7`. It serializes the active space id, `PersistedSettings`, `PersistedInventory`, and `OthelloSettings`. The name can be misleading unless read together with the schema: it stores more than the local player transform because v3.6 separates stable user preferences and hotbar branches from world state. Its `from_dict()` path also accepts earlier input and normalizes missing or malformed settings into defaults."
      ),
      paragraph(
        "`WorldStateFile` is the world-side envelope. Its current version is `3`. It serializes a `spaces` mapping containing `my_world` and `othello`, with each branch carrying player state, world state, and AI actors, and the Othello branch additionally carrying `OthelloGameState`. It also contains migration behavior for earlier files that had top-level `player` and `world` fields without a `spaces` object. Such files are converted into the My World branch while Othello receives a default persisted space."
      ),
      paragraph(
        "`PersistedSettings` is the application preference schema. It stores view parameters, axis inversion, selection outline, cloud state, wireframe flags, shadow state, sun orientation, creative mode, repeated-interaction timing, break-particle controls, auto-jump, auto-sprint, HUD and hand visibility, player name, crosshair mode and pixels, player-skin kind, camera perspective, fullscreen state, view bobbing, camera shake, first-person arm limits, animated texture state, movement parameters, render distance, debug flags, v-sync, HUD debug visibility, window geometry, keybind settings, and audio settings. The schema is intentionally broad because these settings must survive relaunch."
      ),
      paragraph(
        "`PersistedInventory` stores four hotbar branches: creative, survival, Othello, and route-edit. Each branch carries slot contents and selected index, normalized through the domain hotbar size. This is necessary because the same number keys and scroll operations can address different inventory semantics depending on creative mode, current play space, and route-edit state. A single flat hotbar would lose mode-local state when moving between My World, Othello, creative inventory, survival inventory, and AI route editing."
      ),
      paragraph(
        "`PersistedPlayer` stores local player transform and damage state: position, velocity, yaw, pitch, `on_ground`, `flying`, auto-jump cooldown, crouch-eye offset, health, and maximum health. Flying state is restored only when runtime creative-mode state permits it. This avoids converting a previously creative player into an illegal survival flyer while still preserving movement mode in the appropriate context. Player restoration also clamps pitch and clears transient damage cooldowns so saved state remains durable rather than frame-local."
      ),
      paragraph(
        "`PersistedAiPlayer` stores actor identity, AI mode, personality, block-placement permission, held item id, position, velocity, yaw, pitch, health, max health, ground state, flying state, route points, route closed flag, route execution flag, route style, and route target index. It converts to and from `AiPlayerState`, and its `from_dict()` path normalizes unknown modes, personalities, route styles, held items, and coordinates. This makes AI actors first-class persistent domain objects, not renderer decorations or temporary spawn effects."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "`JsonFileStore` provides the low-level JSON persistence operation. It reads UTF-8 text, parses JSON, accepts only mapping objects for application state files, writes compact JSON with sorted keys and `ensure_ascii=False`, flushes and fsyncs a temporary file, and atomically replaces the target path. This store-level discipline matters because player state and world state are user-writable desktop files; partial writes, non-dictionary payloads, encoding drift, and malformed JSON must fail predictably without corrupting later startup."
      ),
      paragraph(
        "`AppStateStore` connects the JSON store to the schema envelopes. It resolves the app-managed data root, constructs `state/player_state.json` and `state/world_state.json`, verifies runtime files when manifest entries exist, falls back to previous-format `configs/player_state.json` and `configs/world_state.json` only when runtime files are absent, converts raw dictionaries into `PlayerStateFile` and `WorldStateFile`, and reassembles an `AppState`. Saving performs the reverse operation and updates the integrity manifest for player and world state files."
      ),
      paragraph(
        "The integrity manifest protects selected runtime files. `PROTECTED_RUNTIME_RELATIVE_PATHS` contains `state/player_state.json`, `state/world_state.json`, `state/player_skin.png`, and `state/othello_opening_book.json`. `_load_or_create_integrity_key()` creates `state/integrity_key.bin` with `secrets.token_bytes(32)` when no valid key exists and attempts chmod `0o600`. `_file_hmac()` computes HMAC-SHA256 over the normalized relative path, a NUL byte, and file bytes streamed in `1024 * 1024` byte chunks. The resulting manifest stores version `1`, algorithm `hmac-sha256`, and a mapping of file paths to digest entries."
      ),
      paragraph(
        "Verification is intentionally conditional. If the file does not exist, verification succeeds; if the manifest does not exist or has no file map, verification succeeds; if the key is missing while a manifest entry exists, verification fails; if a manifest entry is absent for the particular file, verification succeeds; if an entry is present, the stored digest and recomputed digest must compare through `hmac.compare_digest`. This policy detects simple replacement and accidental corruption while preserving migration compatibility and avoiding hard failure on first launch before a manifest has been created."
      ),
      paragraph(
        "Othello opening-book storage uses the same application-persistence boundary. `stores/othello_book.py` normalizes the book root to the app-managed data root, maps the user book to `state/othello_opening_book.json`, maps compiled cache to `cache/othello_opening_book_cache.json`, verifies user-book integrity before loading, writes user-book payloads through JSON, updates the integrity manifest after saving user lines, and registers storage hooks through `configure_opening_book_storage()`. Simulation book logic therefore remains independent of OS-specific runtime-path selection."
      ),
      paragraph(
        "The previous-format `configs/` migration path is not an instruction to keep writing repository-level mutable configuration. It is a compatibility input path. v3.6 writes into the app-managed state root, protects the principal runtime files with the manifest, and keeps cache data separate from durable state. That distinction is essential for desktop packaging because a PyInstaller bundle, a source checkout, a user profile data directory, and a compiled opening-book cache have different mutability and distribution semantics."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "`RuntimePreferences` is the mutable preference vector shared by settings UI, session logic, persistence conversion, renderer runtime state, HUD behavior, and input routing. It contains current play-space id, inversion flags, selection outline, cloud wireframe, cloud enablement, cloud density, cloud seed, cloud-flow direction, world wireframe, shadow enablement, creative mode, four hotbar branches, route-edit state, Othello settings, reach, repeated-action intervals, break-particle controls, auto-jump, auto-sprint, HUD and hand hiding, player name, resolved player name, crosshair mode, crosshair pixels, skin kind, camera perspective, fullscreen flag, view bobbing, camera shake, arm animation limits, animated texture flag, render distance, sun orientation, debug flags, window geometry, keybinds, and audio preferences."
      ),
      paragraph(
        "Normalization is code-level and bounded. Play-space ids are normalized through `normalize_play_space_id`; cloud-flow direction is normalized through `normalize_backend_cloud_flow_direction`; crosshair mode and pixels are normalized through `normalize_crosshair_mode` and `normalize_crosshair_pixels`; camera perspective is normalized through `normalize_camera_perspective`; player-skin kind is normalized through `normalize_player_skin_kind`; player names are normalized to a trimmed maximum length; hotbar slots and indices are normalized through domain hotbar helpers; Othello settings, keybinds, and audio each expose their own normalization path."
      ),
      paragraph(
        "Default timing values are explicit. Block breaking repeats every `0.30` seconds by default and clamps to `0.0..1.0`. Block placing has an initial delay of `0.20` seconds and repeats at `1.0 / 120.0` seconds by default, while legacy persisted `0.20` place intervals are normalized to the current high-frequency value. Block interaction repeats every `0.20` seconds and clamps to `0.0..1.0`. Break-particle spawn rate defaults to `1.0` and clamps to `0.0..2.0`; break-particle speed scale defaults to `1.0` and clamps to `0.1..3.0`."
      ),
      paragraph(
        "Camera and visual preference bounds are equally explicit. Render distance defaults to `6` chunks and clamps to `2..50`. Cloud density clamps to `0..4`; cloud seed clamps to `0..9999`; view bobbing strength and camera shake strength clamp to `0.0..1.0`; arm rotation limits are allowed from `-180.0` to `180.0` degrees; arm swing duration defaults to `6.0 / 20.0` seconds and clamps to `0.05..1.50`; sun azimuth is normalized modulo `360.0`; sun elevation clamps to `0.0..90.0`. Window width and height are clamped to minimum `320 x 240`."
      ),
      paragraph(
        "Keybind settings provide a portable action map rather than relying on platform-native text alone. Default movement binds forward, backward, left, and right to `W`, `S`, `A`, and `D`; jump to `Space`; crouch to `Shift`; sprint to `Control`; inventory to `E`; creative mode to `B`; camera perspective cycle to `F5`; gameplay HUD visibility to `F1`; debug HUD to `F3`; debug shadow to `F4`; clear selected slot to `Q`; and hotbar slots to `1..9`. Bindings reject modifier combinations and normalize through Qt portable key sequences."
      ),
      paragraph(
        "Audio preferences form a four-component gain vector: master, ambient, block, and player. Each component is projected into `[0.0, 1.0]`. `volume_for(category)` returns master gain for the master channel and `master * category_gain` for ambient, block, and player channels. This lets the application keep a global volume scalar while preserving independent category adjustments. Audio preference state is persisted in `PersistedSettings`, restored into runtime preferences, and consumed by presentation audio playback."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "My World is created through `create_my_world_session(seed=0, block_registry=registry)`. The factory wraps `MyWorldSessionSeed`, builds a `SessionManager`, constructs a `PlayerEntity`, supplies `SessionSettings`, creates a world through `make_my_world_state(seed)`, and uses the shared default block registry. The default spawn is `(0.0, 1.0, -10.0)`, yaw is `0.0`, and pitch is `0.0`. These defaults are not UI labels; they are the initial persisted-session state when no saved state overrides them."
      ),
      paragraph(
        "Current My World generation delegates to `generate_test_map(seed)`, which delegates to `generate_flat_world()`. The default flat generator uses `half_extent=32`, `ground_y=0`, and `block_id=minecraft:grass_block`. It iterates from `-32` through `32` on both horizontal axes, yielding a side length of `65` and a base ground area of `4225` block cells before any user edit, gravity transition, AI placement, structural update, or persistence restore. The world revision starts from a generated baseline rather than from an untracked renderer mesh."
      ),
      paragraph(
        "The authoritative world model is `WorldState`, a mutable mapping from integer `(x, y, z)` cells to block-state strings, augmented by revision and index structures. It exposes `set_block()`, `remove_block()`, `set_blocks_bulk()`, `replace_all()`, `get_block()`, `snapshot_blocks()`, `snapshot_for_chunk_build()`, `snapshot_block_window()`, `snapshot_column()`, `chunk_blocks()`, `chunk_revision()`, `to_persisted_dict()`, and `from_persisted_dict()`. A renderer can request snapshots, but the renderer does not own block truth."
      ),
      paragraph(
        "The persisted world branch is `PersistedWorld`, which stores a world revision and a dictionary mapping integer coordinate triples to state strings. Serialization is delegated through `WorldState.to_persisted_dict()` and `WorldState.from_persisted_dict()`, so persisted coordinates, state strings, and revision values remain consistent with the runtime world model. `PersistedPlaySpace` wraps this world state with player state and AI actor state for My World. `PersistedOthelloSpace` applies the same structure to the Othello world branch and adds Othello match state."
      ),
      paragraph(
        "My World is also the space where ordinary block interaction is enabled. Placement, breaking, creative inventory, route editing, AI special-item spawning, falling-block behavior, collision, player damage, camera motion, selection outline, HUD payload, chunk upload, and renderer frame metrics all refer back to the active `SessionManager`. This makes My World a full application mode rather than a passive terrain viewer. Its state must survive save/load, space switching, renderer backend choice, and settings changes."
      ),
      paragraph(
        "The flat fixture should therefore be described carefully. It is a current generation fixture, not an architectural limit that prevents later generation work. The persistence model already supports arbitrary block mappings and revision tracking, while chunk snapshots and renderer uploads operate over the current world mapping. A future generator can populate `WorldState` differently, but the current v3.6 overview must describe the actual flat fixture, its numerical extent, its default block id, and its relation to persistent user edits."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Chunk identity is defined in foundations rather than in a renderer backend. `CHUNK_SIZE = 16`; `chunk_key(x, y, z)` maps block coordinates to a `ChunkKey`; `chunk_origin(chunk_key_value)` reconstructs the world-space chunk origin; `local_coords(x, y, z)` computes local coordinates; `chunk_bounds(k)` returns coordinate bounds; and `neighbor_chunk_keys_for_cell(x, y, z)` determines adjacent chunks affected when an edit occurs on a boundary. This common chunk vocabulary is used by world indexing, structural updates, selection refresh, mesh build snapshots, upload tracking, and renderer eviction."
      ),
      paragraph(
        "World upload is handled by `WorldUploadTracker`, which mediates between authoritative `WorldState` and backend-resident render chunks. It tracks submitted revisions, schedules chunk build work, holds pending futures, drains ready results, caches build output, evicts chunks no longer inside the keep set, and exposes whether the initially needed chunk set is resident. This makes loading completion observable through renderer residency instead of through a fixed delay or the mere existence of a window."
      ),
      paragraph(
        "The visible chunk set is a function of eye position and render distance. The runtime default render distance is `6` chunks and is clamped to `2..50`, so a normal frame considers horizontal chunk neighborhoods around the player while preserving bounded resource demand. The upload path can scan surrounding chunks, prioritize by distance, and retain CPU-side build results across play-space switches so that returning to a previously visited space does not necessarily require rebuilding every mesh from scratch."
      ),
      paragraph(
        "Chunk submission uses the renderer contract. Backends receive `submit_chunk(chunk_key=..., world_revision=..., faces=..., shadow_faces=..., gpu_face_sources=..., gpu_bucket_counts=...)`. This call separates world mutation, CPU face construction, backend-specific buffer upload, shadow-face availability, and draw-resident chunk revision. If a block changes, the affected chunk and neighboring boundary chunks can be marked dirty while unchanged chunks remain resident. This is a fundamental performance and correctness boundary for a persistent voxel application."
      ),
      paragraph(
        "Selection also depends on chunk state. `set_selection_target()` receives a block coordinate, state string, state access function, and world revision. That means the outline, pick AABBs, and render boxes for partial block families can be refreshed against the actual world revision rather than a stale renderer-side copy. Selection clearing is an explicit renderer API operation so modal states, space switching, player death, and unsupported target states can remove selection without mutating the world."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Player motion is domain logic rather than Qt input code. The movement system is parameterized through `MovementParams`, `CollisionParams`, and `SessionSettings`. Default movement values include `tick_hz = 20.0`, walk speed `4.317`, sprint speed `5.612`, crouch multiplier `0.3`, gravity `32.0`, maximum fall speed `78.4`, jump velocity `8.4`, ground acceleration `30.0`, air acceleration `6.0`, sprint-jump boost `5.0`, auto-jump probe `0.35`, auto-jump success delta `0.90`, auto-jump cooldown `0.12`, fly speed `10.92`, fly ascend speed `10.92`, and fly descend speed `10.92`. These numbers are restored through settings and runtime preferences rather than duplicated in widgets."
      ),
      paragraph(
        "Collision defaults are similarly explicit. Epsilon is `1e-4`, ground probe is `0.03`, step height is `0.5625`, nearby XZ padding is `1`, downward Y padding is `2`, upward Y padding is `1`, and sneak step is `0.05`. Collision uses AABBs derived from block states and family-specific geometry. A slab, stair, fence, fence gate, or wall can therefore block or support the player differently from a full cube. Depenetration is required because restored state, block placement, gate closure, and falling-block settlement can create temporary overlap that must be resolved according to the same domain rules."
      ),
      paragraph(
        "Player persistence stores movement-relevant state. `PersistedPlayer` records position, velocity, yaw, pitch, `on_ground`, `flying`, auto-jump cooldown, crouch-eye offset, health, and maximum health. During restore, the runtime player receives those values, pitch is clamped, flying is retained only when creative mode permits it, transient cooldowns are reset where appropriate, and overlap exemptions are reconstructed for closed fence gates and gravity blocks. This lets saved state restore into a legal simulation state instead of a purely serialized coordinate dump."
      ),
      paragraph(
        "Damage is also part of the domain model. The normal health pool is `20.0`. Melee attack damage is `1.0`; melee cooldown is `0.5` seconds; hurt flash and tilt timings are represented separately; void damage begins below `y = -64.0`, repeats every `0.5` seconds, and applies `4.0` damage per tick. Local player-to-AI targeting uses a melee reach of `3.0` blocks. Death and respawn must interact with camera, HUD, pause state, renderer tint, and persistence without redefining health inside presentation code."
      ),
      paragraph(
        "The fixed-step runner coordinates simulation advancement. The desktop game loop has a `sim_hz` of `120.0`, while movement constants also retain a Minecraft-like reference tick of `20.0`. This distinction lets repeated placement and input sampling operate at desktop-loop cadence while preserving domain constants used by movement, gravity, and animation. The result is a layered time model: Qt event delivery, fixed-step session advancement, renderer frame submission, particle age, falling-block tick, and Othello clocks can each remain explicit."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Block interaction is routed through `InteractionService` and session manager methods rather than through direct viewport mutation. The interaction path handles picking, breaking, placing, fence-gate toggling, held placement, player-overlap rejection, structural neighbor updates, gravity conversion, break-particle emission, and world-revision updates. The viewport supplies input and targeting conditions; the simulation layer determines whether a requested action is legal and what world edits result."
      ),
      paragraph(
        "Breaking and placing have independent repeat semantics. Breaking defaults to a `0.30` second repeat interval. Placement has a `0.20` second initial dwell before held repetition and then repeats at `1.0 / 120.0` seconds. Interaction for toggles such as fence gates defaults to `0.20` seconds. These timing values are persisted through runtime settings and influence how quickly the world can change under continuous input. They also determine how route-frontier state, collision correction, and renderer uploads react when the user holds a mouse button."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Gravity is a simulation rule under `simulation.rules.gravity.system`. Blocks with `GRAVITY_AFFECTED_TAG = gravity_affected` can convert from static world cells into falling-block samples when support fails. The falling tick is `1.0 / 20.0` seconds; gravity per tick is `0.04`; drag is `0.98`; epsilon is `1e-6`. The system removes unsupported blocks from `WorldState`, advances transient falling samples, tests landing against collision and support conditions, and writes settled blocks back into the world when landing is valid."
      ),
      paragraph(
      ...
      paragraph(
        "Falling-block rendering is a DTO problem. Simulation emits falling sample state, and presentation rendering turns it into face rows using the same atlas and block-state-to-render-box conventions as ordinary blocks. `FallingBlockRenderSampleDTO` carries enough state for the renderer to place and orient the moving block. This avoids giving the renderer authority to decide when a falling block lands or when it should be removed from world state."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "AI players are first-class runtime actors. They are not path lines, preview ghosts, or renderer-only meshes. The AI subsystem contains state, settings, spawning, serialization, manager coordination, idle behavior, wandering, route following, navigation, parkour, recovery, stuck detection, avoidance, combat, placement, route planning, background worker logic, and render-state extraction. The `AiPlayerState` object carries transform, velocity, behavior mode, personality, health, route information, and placement permission, and `PersistedAiPlayer` stores the durable representation."
      ),
      paragraph(
        "Spawning is performed through the AI special item. The current item id can resolve to `ludoxel:ai_spawn_egg`, and the session manager can call `spawn_ai_player_for_session(session, spawn_cell=..., settings=...)`. The resulting actor receives per-instance settings, an actor id, initial state, and a persisted identity. This route preserves the distinction between an inventory item, a click interaction, a spawn cell, actor settings, and the AI manager’s internal state."
      ),
      paragraph(
        "AI modes normalize to idle, route, and wander. User-facing labels can correspond to Standby, Route Patrol, and Free Roam / PVP. Personalities normalize to aggressive or peaceful. Route styles normalize to strict or flexible. Placement permission is per actor through `can_place_blocks`, and held item defaults to a block id such as `minecraft:oak_planks`. Defaults include mode `idle`, personality `aggressive`, health `20.0`, maximum health `20.0`, and route style `strict`. These defaults matter because missing or malformed persisted state must restore to a valid actor rather than crashing the session."
      ),
      paragraph(
        "AI persistence encodes route behavior. Route points are stored as `AiRoutePoint` triples, together with route closure, route execution state, route style, and target index. A closed route loops; an open route can traverse and return according to route logic. Persisting the target index means an actor can resume route progression rather than restarting from the first authored point every time the application loads. The route branch of the hotbar and route overlay are presentation tools layered over this domain state."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      ...
      paragraph(
        "The planner uses explicit numeric limits. Maximum support Y delta is `1`; parkour search cap is `8`; parkour sample count is `5`; drop search depth is `4`; visit limits range from `1024` to `4096`; target support search radius is `6`. These bounds prevent route planning from becoming an unbounded search across the entire world while still permitting local detours, drops, jumps, and support-cell repair. The route planner therefore has a bounded-world snapshot semantics rather than direct infinite-world authority."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Camera perspective is a persisted preference with a finite order: first person, third person back, and third person front. The default is first person, and the default cycle action is bound to `F5`. Third-person camera placement is constrained by collision checks so that the camera retracts before penetrating world geometry. First-person rendering uses view-model visibility and hand hiding rules, while third-person rendering exposes the player model, name-tag placement, held item state, and shadow-caster alignment."
      ),
      paragraph(
        "HUD behavior is stateful. Gameplay HUD visibility can be toggled with `F1`; debug HUD can be toggled with `F3`; debug shadow can be toggled with `F4`; Othello HUD visibility respects ordinary HUD suppression and debug HUD precedence. The HUD consumes payloads rather than directly reading every simulation object. It can display metrics, crosshair, hotbar state, route overlays, Othello evaluation, Othello turn state, player health, and debug information, while modal overlays such as pause, inventory, death, settings, and skin preview can suspend gameplay input."
      ),
      paragraph(
        "The skin pipeline stores skin kind as `alex` or `custom`, persists imported custom skin as `state/player_skin.png`, and protects that file through the integrity manifest. The renderer contract includes `set_player_skin_image()` and `render_player_preview_frame()`, allowing both the in-world player and the pause preview to use the same skin texture state. A modern player skin uses the `64 x 64` texture convention, and visual modules construct first-person arms, third-person body parts, UV maps, pose state, held block geometry, and player render state from that skin data."
      ),
      paragraph(
        "Crosshair customization is stored as a `16 x 16` binary grid. The default mode is `default`; custom mode uses the persisted pixel rows. `normalize_crosshair_pixels()` accepts list or tuple input, truncates or pads rows to `16`, maps only `1` to an active pixel, and fills missing rows with zeros. This means the crosshair editor stores a compact deterministic representation rather than an arbitrary image file. HUD drawing can then render the crosshair according to first-person and HUD-visibility conditions."
      ),
      paragraph(
        "Audio preference state is normalized through `AudioPreferences`. The categories are master, ambient, block, and player. Each volume is clamped to `[0.0, 1.0]`. Master gain is returned directly for the master channel, while ambient, block, and player effective gains multiply the master value by the category-specific value. This structure allows one global attenuation control while preserving per-category mixing semantics. Presentation audio playback can then treat ambient loops, block sounds, and player sounds differently while respecting one persisted preference record."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "The Othello play space is created through `create_othello_session(seed=0, block_registry=registry)`. Its default spawn is `(0.0, 1.0, -12.0)`, yaw is `0.0`, and pitch is `0.0`. The world factory generates a flat world with `half_extent=48`, `ground_y=0`, and `minecraft:grass_block`, then applies `ensure_othello_board_layout(world)`. The base generated floor spans `-48..48` on both horizontal axes, giving side length `97` and `9409` ground cells before the board layout is applied."
      ),
      paragraph(
        "The Othello board has `BOARD_SIZE = 8` and `BOARD_CELL_COUNT = 64`. Board footprint coordinates run from `x=-4..3` and `z=-4..3`; board blocks occupy `y=1`; the board surface is at `y=2.0`. The board uses dark and light wood-like blocks for the square pattern. Square names are produced in algebraic form from `a1` through `h8`. A board raycast maps a world-space ray hit to a square index, and square-index conversion functions map between row/column, index, and visible square name."
      ),
      paragraph(
        "`OthelloGameState` stores match status, board tuple, settings, player side, AI side, current turn, black clock, white clock, move count, consecutive passes, winner, message, last move, animations, match generation, legal moves, and thinking state. Status values include idle, player turn, AI turn, animating, and finished. Sides are encoded as empty `0`, black `1`, and white `2`. Draw state is represented separately from either side. This explicit game-state object makes the Othello match persistable and renderable independently of the current viewport frame."
      ),
      paragraph(
        "The initial board contains the standard `2 x 2` center discs. Legal move logic computes captures, legal moves, pass conditions, and winner determination from the board representation. Disc placement is performed by aiming at the board and interacting with a legal square. The renderer can then update board visuals, HUD evaluation, move animations, and clock state while simulation rules continue to own legality. Othello therefore remains a domain game inside the same 3D world rather than a separate 2D widget detached from persistence."
      ),
      paragraph(
        "Othello hotbar controls are domain-specific. Slot `0` contains the start item, and slot `8` contains the settings item. These items can be rendered as held controls in first-person and third-person contexts while still being special items rather than placeable blocks. Othello mode disables ordinary block placement and block breaking, but it preserves shared shell behavior, pause behavior, settings behavior, renderer contract usage, and persistent player state."
      ),
      paragraph(
        "Othello animation is also explicit. Animation modes include off, fast, and slow. Flip animation carries square index, from side, to side, elapsed time, duration, delay, and lift height. Default flip duration is `0.22`, and lift height is `0.075`. These values let the renderer animate disc transitions while the match state remains discrete and rule-driven. The animation path must not create illegal board state; it visualizes a transition determined by rules."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "`OthelloSettings` is a bounded configuration record for the match and engine. Difficulty values are `weak`, `medium`, `strong`, `insane`, and `insane_plus`. Time-control values include `off`, `per_move_5s`, `per_move_10s`, `per_move_30s`, `per_side_1m`, `per_side_3m`, `per_side_5m`, `per_side_10m`, and `per_side_20m`. Animation modes include `off`, `fast`, and `slow`. Player side can be black or white. These settings are persisted through `PlayerStateFile` and restored into runtime preferences."
      ),
      paragraph(
        "Time controls map to explicit seconds. Per-move controls map to `5.0`, `10.0`, and `30.0` seconds. Per-side controls map to `60.0`, `180.0`, `300.0`, `600.0`, and `1200.0` seconds. The default time-control option is `per_side_20m`, giving `1200.0` seconds. The default time limit constant is also `20.0 * 60.0`. Match clocks are part of Othello game state and must pause under modal conditions such as pause and detached Othello settings windows."
      ),
      paragraph(
        "Engine configuration includes sacrifice level, worker count, hash level, and opening-book learning parameters. Sacrifice level clamps to `0..4`. Worker count clamps to `1..8`. Hash level clamps to `0..6`. Book learning depth clamps to `0..60`. Per-move, cumulative, and leaf error bounds clamp to `0.0..24.0`. Defaults include medium difficulty, black player side, sacrifice level `2`, worker count `1`, hash level `2`, learning depth `55`, per-move error `22.0`, cumulative error `19.0`, and leaf error `20.0`."
      ),
      paragraph(
        "The engine modules are split by responsibility. Bitboard code represents board state compactly; classic rules provide ordinary legal-move and capture behavior; evaluation profiles define scoring coefficients; evaluation computes position values; ordering ranks moves for search; transposition caches search states; worker modules isolate long-running AI evaluation from the UI; and insane-mode code selects stronger search behavior. `Insane+` can consult opening-book data before falling back to the same deeper search family."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Othello opening-book storage is divided between bundled resources, user state, and compiled cache. Bundled opening-book data lives under `simulation/spaces/othello/resources/opening_book.json`, which is included as package data. User-created or learned book lines are stored under the app-managed state path `state/othello_opening_book.json`. The compiled lookup cache is stored under `cache/othello_opening_book_cache.json`. This separation allows bundled data to remain immutable, user extensions to remain durable, and compiled structures to remain disposable cache."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      ...
      paragraph(
        "Shader resources are package-managed. The OpenGL shader root includes `chunk_face_payload.comp`, `cloud_box.frag`, `cloud_box.vert`, `first_person_face.frag`, `first_person_face.vert`, `othello_shadow.vert`, `othello.frag`, `othello.vert`, `player_model_no_shadow.frag`, `player_model_shadow.vert`, `player_model.frag`, `player_model.vert`, `selection_line.frag`, `selection_line.vert`, `shadow.frag`, `shadow.vert`, `sun.frag`, `sun.vert`, `world_no_shadow.frag`, `world.frag`, `world.vert`, and `common/face_instance.glsl`. These files are included by both `MANIFEST.in` and package-data metadata."
      ),
      paragraph(
        "The compute shader `chunk_face_payload.comp` is central to the OpenGL chunk payload path. It belongs to the OpenGL backend’s resource model, not to the abstract renderer contract. A project overview must therefore say that OpenGL can use the compute-backed path where available, while the backend-neutral contract remains expressed through chunk submission, face rows, shadow faces, and world-build tools. This distinction is necessary for macOS parity because the wgpu backend has a different runtime and pipeline model."
      ),
      paragraph(
        "OpenGL shadow behavior uses backend defaults such as shadow map size `2048`, shadow dark multiplier `0.20`, bias values `0.00005` and `0.00050`, sun azimuth `45.0`, sun elevation `60.0`, sun distance `150.0`, light distance `60.0`, and orthographic radius `30.0`. These values matter because world pass, player model pass, first-person pass, special item rendering, cloud rendering, and Othello rendering need a coherent scene lighting and shadow basis."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "The macOS renderer path uses the wgpu backend under `presentation.rendering.backends.wgpu`. It is organized into chunk meshes, pipeline factory functions, runtime backend state, runtime resources, surface configuration, texture atlas handling, and shader source adaptation. It is selected through the presentation shell on Darwin and is intended to avoid dependence on an OpenGL 4.3 context on macOS. The backend operates through the renderer contract so shell code and settings surfaces can remain backend-neutral."
      ),
      paragraph(
        "`configure_wgpu_canvas(canvas, adapter, device)` is the surface configuration point. It obtains the `wgpu` canvas context, asks for the preferred adapter format, and configures the surface with render-attachment usage and opaque alpha mode. The runtime uses depth format `depth24plus`, a `44`-float uniform vector, a `176`-byte uniform buffer size, and a clip-space transform that adapts OpenGL-style projection conventions to wgpu depth conventions. These are backend implementation details, but they define how macOS frames become visible."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Gameplay input is owned by presentation input modules and viewport lifecycle code. `InputFrame` represents the state consumed by gameplay: forward movement, strafe movement, jump held state, jump pressed edge, sprint, crouch, and mouse deltas. `QtInputAdapter` maps Qt key events and keybind settings into this frame. `ViewportInput` owns capture enablement, mouse and keyboard grab state, blank cursor application, cursor recentering, delta clearing, keyboard focus, viewport activation, and capture release. This ownership prevents ordinary widget focus from silently becoming the gameplay control model."
      ),
      paragraph(
      ...
      paragraph(
        "macOS input capture uses additional helpers. `MacosGameplayInputGuard` installs a CoreGraphics event tap for key down, key up, and flags changed events; maps platform keycodes to Qt key semantics; and reports installation status when permission is missing. `MacosCursorWarp` wraps cursor warping and cursor association. The `.app` bundle declares `NSInputMonitoringUsageDescription` because gameplay key capture on macOS can require Input Monitoring or Accessibility permission. This macOS path is explicit because Qt shortcut handling and OS-level function-key behavior can otherwise preempt gameplay input."
      ),
      paragraph(
        "Mouse recapture includes a zero-delta phase. After capture is enabled or restored, the viewport recenters the cursor, ignores a short burst of mouse-move events, clears accumulated deltas, and waits for stable center observations before ordinary mouse-look resumes. Constants include an ignored-move duration of `0.025` seconds, ignored move event count `2`, stable center tolerance `1.0` pixel, and `2` stable polls before resuming. Keyboard movement can continue during this phase while camera delta is suppressed."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      ...
      paragraph(
        "Overlays are modal or semi-modal surfaces with simulation consequences. Pause can suspend input and clocks; death overlay can coordinate respawn; inventory can expose block and item selection; AI settings can update actor settings; skin preview can import and reset player skin; Othello settings can change match and engine configuration. These surfaces must integrate with `save_state()`, runtime preferences, input capture, renderer pause state, and application shutdown. They are therefore part of the application contract, not ornamental dialogs."
      ),
      paragraph(
        "The About page is implemented through `AboutBlock`, `AboutSection`, `paragraph()`, `code_block()`, `render_about_sections()`, and About widget helpers. `AboutBlock` carries `kind` and `text`; `AboutSection` carries `title` and tuple-valued `blocks`. `paragraph(text)` returns a paragraph block; `code_block(text)` returns a code block; the renderer creates cards, section titles, paragraph labels, and code blocks. This means `ABOUT_PROJECT_OVERVIEW_SECTIONS` must be valid Python data compatible with the existing renderer rather than an arbitrary Markdown document."
      ),
      paragraph(
        "Creator-profile content and project-overview content have distinct roles. `ABOUT_PROFILE_BIO_TEXT`, `ABOUT_WORK_TEXT`, `ABOUT_ACADEMIC_DIRECTION_TEXT`, and `ABOUT_ETYMOLOGY_PARAGRAPHS` describe creator-facing biographical, academic, and naming material. `ABOUT_PROJECT_OVERVIEW_SECTIONS` describes the implemented software system. It can and should include build commands, package metadata, renderer resources, persistence files, legal files, and desktop packaging because those are actual project boundaries visible in code, package configuration, and runtime behavior."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "The repository command surface is part of the project specification. It is declared in `package.json` and implemented through `tools/`. The command surface includes help rendering, formatting, linting, project checks, documentation checks, license checks, resource checks, shader checks, directory export, native-extension builds, desktop application builds, cleanup, and audio asset conversion. These commands are not external notes: they determine whether code, resources, package data, shader files, legal files, and desktop packaging are verifiable from the repository."
      ),
      paragraph(
        "Core npm scripts include `help`, `package:check`, `docs:check`, `license:check`, `resources:check`, `shader:check`, `lint`, `lint:js`, `lint:css`, `lint:py`, `format`, `format:web`, `format:py`, `format:check`, `format:web:check`, `format:py:check`, `tools:export`, `tools:export:help`, `tools:export:src`, `tools:export:root`, `tools:export:archive`, `tools:test`, `docs:export`, `build:desktop`, `build:desktop:help`, `build:windows`, `build:windows:help`, `build:macos`, `build:macos:help`, `build:macos:check`, `build:native`, `build:native:check`, `clean`, `clean:check`, `assets:audio:convert`, `assets:audio:check`, `check`, and `ci`. The `check` script composes formatting, linting, tool tests, package checks, documentation checks, license checks, resource checks, and shader checks."
      ),
      paragraph(
        "The Node metadata sets `type` to `module`, marks the package private, and declares engines `^20.19.0 || ^22.13.0 || >=24`. Development dependencies include `@eslint/js` `10.0.1`, `eslint` `10.4.0`, `globals` `17.5.0`, `prettier` `3.8.3`, `stylelint` `17.12.0`, `stylelint-config-standard` `40.0.0`, and `vite` `^6.4.2`. Those versions matter because web-source formatting, JavaScript linting, CSS linting, and Node script execution depend on the declared toolchain."
      ),
      paragraph(
        "Python formatting policy is declared in `pyproject.toml`. Ruff uses line length `200`, indent width `2`, target version `py313`, lint selection `E9`, `F63`, `F7`, and `F82`, double quote style, space indentation, `skip-magic-trailing-comma = true`, and disabled docstring-code formatting. Two-space Python indentation is not a casual style note; it is declared in the formatter configuration and must be compatible with the actual code emitted into the repository."
      ),
      paragraph(
        "Package metadata and command checks interact with package resources. Shader validation must understand OpenGL shader files and wgpu shader sources. Resource validation must understand QSS, Othello JSON resources, assets, fonts, and third-party material. License validation must include `LICENSE`, `NOTICE`, and third-party notices. Build commands must include package data, legal material, and platform-specific renderer dependencies. The command surface therefore protects runtime correctness as much as formatting style."
      ),
      paragraph(
        "Directory export tooling is also part of development workflow. It can output repository contents for inspection, and its own scripts appear under `tools/export_directory_markdown`. That matters because tasks such as this overview audit rely on exported source rather than memory of a README. The application’s About text should be able to be checked against the same exported code and package metadata that project tools can produce."
      ),
      code_block(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Desktop packaging is implemented under `tools/build_desktop_app` and declared through package scripts such as `build:desktop`, `build:windows`, `build:macos`, and `build:macos:check`. The tool directory contains platform run scripts, argument parsing, validation, help rendering, dispatch, native-build command wiring, PyInstaller command construction, legal-copy service, macOS build service, macOS status service, Windows build service, task service, path configuration, file helpers, process helpers, and Python resolution. Packaging is therefore an auditable repository tool, not an external checklist."
      ),
      paragraph(
        "Package resource inclusion is declared in both `MANIFEST.in` and package-data metadata. `MANIFEST.in` grafts `assets`, `third-party`, and `tools`; includes `pyproject.toml`, `README.md`, `LICENSE`, and `NOTICE`; recursively includes theme QSS, OpenGL shader files, OpenGL common GLSL files, wgpu shader-source files, wgpu common GLSL files, and Othello JSON resources; and globally excludes Python bytecode. Package-data metadata mirrors QSS, OpenGL shaders, wgpu shader sources, and Othello resources under the `ludoxel` package. This duplication makes resource availability visible to both source distribution and installed package paths."
      ),
      paragraph(
  ...
  AboutSection(
    ...
    blocks=(
      paragraph(
        "Ludoxel Original Materials are governed by the Ludoxel Independent License in `LICENSE` under `LicenseRef-All-Rights-Reserved`. The license text identifies version `1.0.1`, original effective date `2026-06-03`, current version effective date `2026-06-04`, and states that the repository is not open source and not free software. It also states that the original materials are not licensed under Apache-2.0, MIT, BSD, GPL, AGPL, LGPL, MPL, or other open-source licenses. This legal status belongs in the project overview because it controls distribution, copying, reuse, derivative works, and training-data use."
      ),
      paragraph(
        "The license distinguishes original materials, third-party materials, user materials, and application output. Original materials include source code, documentation, configuration, build scripts, repository text, UI text, generated data, images, metadata, packaged files, distribution files, shaders, bundled resources, desktop app packaging material, and other repository contents created by Kento Konishi. Third-party libraries, fonts, external packages, vendor files, platform SDKs, and materials identified in `NOTICE` or `third-party/` remain under their own terms. User materials and ordinary application output do not become original project materials merely because the application saved, displayed, or exported them."
      ),
      paragraph(
        "`NOTICE` records the relationship between Ludoxel original materials, third-party material, provenance-sensitive local assets, runtime user data, and distribution legal material. It identifies Kaisei Opti as a third-party font under SIL Open Font License `1.1`, with corresponding notice and license files under `third-party/kaisei-opti/`. It also identifies Minecraft-named local assets and fonts under `assets/` as materials whose provenance, rights, and redistribution status are not established by the repository. Those assets must not be described as Ludoxel original materials."
      ),
      paragraph(
