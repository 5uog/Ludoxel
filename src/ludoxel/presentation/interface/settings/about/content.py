...
ABOUT_PROJECT_OVERVIEW_SECTIONS: tuple[AboutSection, ...] = (
  ...
  AboutSection(
    ...
    blocks=(
      ...
      paragraph(
        ...
        text_run(
          ", with Off as the default. The hearts use the same pixel pattern and fill rules as the player's Survival health strip: one heart equals two health points, a half heart equals one health point, and 20 health points render as ten hearts in one row. The indicator is placed so that it does not overlap the nametag, follows the same screen clamping and occlusion dimming as the world nametag, and is drawn by Qt overlay widgets shared by the Windows OpenGL and macOS wgpu viewport paths. The nametag and hearts are composed into one overlay block, then the entire block, including its background, padding, text, hearts, and spacing, is scaled from the camera-to-AI world distance. Distant AI therefore render a physically smaller status block, and tags are hidden entirely beyond 64 blocks."
        ),
      ),
      paragraph(
        text_run(
  ...
  AboutSection(
    ...
    blocks=(
      ...
      paragraph(
        text_run(
          "The planner receives a full-map snapshot. The AI manager copies the entire world block dictionary into an immutable snapshot once per world revision and reuses that snapshot for every plan request, so neither actor movement nor repeated requests rebuild it on the simulation thread. Planner search bounds are derived from the snapshot's own x/z extents with a small padding rather than from a radius around the start and target, which removes the earlier narrow-window limitation that caused unreachable detours to be missed. The remaining numeric limits are explicit: maximum support Y delta is "
        ),
        code_run("1"),
        ...
        code_run("4"),
        text_run("; the visit limit scales with the map's column count between "),
        code_run("1024"),
        text_run(" and "),
        code_run("50000"),
        text_run("; target support search radius is "),
        code_run("6"),
        text_run(". The large visit limit is acceptable because the search runs in a separate worker process and never blocks the frame."),
      ),
      paragraph(
        "Flexible routes and strict routes differ in how they react to navigation failure. A strict route preserves authored intent: the actor walks directly toward each route point, does not search for detours, and getting stuck against blocks is accepted behavior. A flexible route asks the planner for a support-cell route across the full-map snapshot in a background worker process and follows the resulting path when complete; it can detour anywhere the map permits, recover from blocked spots, and use block placement for traversal when placement is allowed. Failed plans retry with exponential backoff, and a route point that keeps failing is placed on a per-actor cooldown blacklist and temporarily skipped in favor of the next route point instead of being retried without limit. If every route point is currently blacklisted, the actor holds position and turns toward the target until a cooldown expires."
      ),
      paragraph(
        "Plans react to world changes. When the world revision changes through block placement, block breaking, fence-gate toggles, gravity settlement, or bulk edits, the manager revalidates the remaining steps of the cached plan: walk steps must still be standable and placement steps must either already be filled with standable support or remain open with clear headroom. A step invalidated by the change discards the plan without blacklisting anything and requests a fresh full-map plan immediately, so a path closed by the player is replanned and, when placement is allowed, can be reopened by bridging instead of being retried blindly."
      ),
      paragraph(
        "The background worker exists because route planning can be more expensive than a single frame. Worker logic receives the full-map snapshot, route point data, actor dimensions, and route settings, then computes route results without blocking the Qt event loop; route planning never touches presentation rendering state. Main-thread search work is bounded on several axes: the breadth-first local recovery search caches its result per actor, is limited to one fresh search per simulation step, carries an explicit per-search time budget, and restricts its escape parkour span; the reachable support cell for the current route target is cached per world revision instead of being rescanned every step; and the direct walk-line shortcut test is skipped beyond a fixed span so distant targets go straight to the planner. The application must still preserve synchronization: world edits, actor movement, route cancellation, space switching, and shutdown can invalidate or supersede worker results. Therefore route planning is background work, but route application remains manager-owned."
      ),
      paragraph(
        "Navigation includes stuck detection and recovery. An actor following a route or pursuing a player can become blocked by changed blocks, partial geometry, gravity-settled blocks, player placement, or route edits. Stuck detection, avoidance, recovery, and parkour modules provide corrective behavior, including escape candidates from boxed-in positions and placement-based steps when the actor may place blocks. Waiting in front of a valid pending bridge placement is not treated as being stuck: while the next plan step is a placement whose conditions still hold, the step and goal stuck timers are suspended so the actor does not blacklist its own bridge while the placement cooldown elapses. The AI subsystem can cancel navigation for an actor, remove actors, update actor settings, and expose route path snapshots for the HUD and overlay layers. These controls are necessary because route editing is user-driven and persistent."
      ),
      paragraph(
      ...
      paragraph(
        "Block placement is treated as a movement aid with explicit safety rules. Before placing, the AI verifies a clear line of sight from its eye position to the targeted placement face using the picking ray; if another block model obstructs that ray, the placement is cancelled rather than placed through the obstruction. Bridge placement secures the next footing first, the edge-safety gate keeps the actor from advancing ahead of an unfinished bridge, and successive bridge blocks follow at the placement cadence of one block per 0.30 seconds while the suspended stuck timers keep the plan alive, so the actor lays a continuous bridge instead of freezing after the first block. A wandering actor that can place blocks holds its heading at an edge and bridges instead of turning away. When placement is disabled, the actor never places blocks and prefers stopping or turning away over falling. Combat pursuit interacts with these rules as before: aggressive AI may suspend route planning during melee pursuit, peaceful AI avoids the combat path, and placement permission, mode, personality, route style, health, and route target index are persisted together."
      ),
      code_block(
        "support-cell route planning definition:\n  route search over physically supportable actor cells\n  route points converted to reachable support cells\n  full-map world snapshot, cached per world revision\n  planner bounds: snapshot x/z extents plus padding\n  route result applied by AI manager\n\nplanner numeric bounds:\n  max support Y delta: 1\n  parkour search cap: 8\n  parkour sample count: 5\n  drop search depth: 4\n  visit limit: 1024 to 50000, scaled by map column count\n  target support search radius: 6\n\nsearch and retry budgets:\n  route plan requests per step: 1\n  local recovery searches per step: 1\n  local recovery result cache: 0.30 s\n  local recovery time budget: 1.5 ms per search\n  local recovery escape parkour span: 3\n  route target support cache: per world revision\n  direct walk-line test span cap: 24 cells\n  failed target blacklist after: 3 consecutive plan failures\n  failed target cooldown: 8.0 s\n\nworld-change reaction:\n  remaining plan steps revalidated on every world revision change\n  invalidated plans replan immediately without blacklisting\n\nmovement safety:\n  forward footing check before ground steps\n  free roam / PVP safe drop: up to 3 blocks\n  route-following drop window: up to 8 blocks with support below\n  placement line-of-sight check via picking ray\n  bridge placement secures next footing before advancing\n  bridge cadence: one block per 0.30 s without stuck penalties\n\nroute behavior controls:\n  route_closed\n  route_run\n  route_style: strict or flexible\n  route_target_index\n\nmanager operations:\n  update actor settings\n  validate AI display names\n  cancel actor navigation\n  remove actor\n  pick actor by ray\n  attack actor from local player\n  expose route path snapshots\n  expose render snapshots"
      ),
    ),
  ),
  AboutSection(
