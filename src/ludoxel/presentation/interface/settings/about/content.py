...
ABOUT_PROJECT_OVERVIEW_SECTIONS: tuple[AboutSection, ...] = (
  ...
  AboutSection(
    ...
    blocks=(
      ...
      paragraph(
        "The Node metadata sets `type` to `module`, marks the package private, and declares engines `^20.19.0 || ^22.13.0 || >=24`. Development dependencies include `@eslint/js` `10.0.1`, `eslint` `10.4.1`, `globals` `17.6.0`, `prettier` `3.8.3`, `stylelint` `17.12.0`, `stylelint-config-standard` `40.0.0`, and `vite` `^6.4.2`. Those versions matter because web-source formatting, JavaScript linting, CSS linting, and Node script execution depend on the declared toolchain."
      ),
      paragraph(
