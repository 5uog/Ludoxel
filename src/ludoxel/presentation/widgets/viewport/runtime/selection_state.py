...
# FILE: src/ludoxel/presentation/widgets/viewport/runtime/selection_state.py
...
class ViewportSelectionState:
    ...
    def _make_key(
        ...
        *,
        ...
        return (
            round(float(eye_vec.x), 4),
            round(float(eye_vec.y), 4),
            round(float(eye_vec.z), 4),
            round(float(yaw), 3),
            round(float(pitch), 3),
            int(session.world.revision),
            round(float(reach), 3),
        )
    ...
    def refresh(
        ...
        *,
        ...
        hit = pick_block(
            session.world,
            origin=origin,
            direction=forward_from_yaw_pitch_deg(yaw, pitch),
            reach=float(reach),
