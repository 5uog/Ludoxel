...
from ludoxel.presentation.rendering.visuals.players.model_pose import HeldBlockPose, PlayerModelPose
  ...
  def set_ai_skin_images(self, images: dict[str, QImage]) -> None:
    ...
    try:
      for skin_key, image in images.items():
        next_textures[str(skin_key)] = ImageTexture.from_image(QImage(image))
    except Exception:
