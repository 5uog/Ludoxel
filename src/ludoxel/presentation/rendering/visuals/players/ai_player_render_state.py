...
def compose_ai_player_render_states(snapshots: tuple[AiPlayerRenderSnapshotDTO, ...], *, block_registry: BlockRegistry) -> tuple[PlayerRenderState, ...]:
  ...
  for snapshot in snapshots:
    ...
    motion = FirstPersonMotionSample(
      visible_item_id=held_item_id, target_item_id=held_item_id, equip_progress=1.0, prev_equip_progress=1.0, swing_progress=float(snapshot.attack_swing_progress), prev_swing_progress=float(snapshot.attack_prev_swing_progress), show_arm=bool(held_item_id is None), show_view_model=False, slim_arm=True
    )
    states.append(compose_player_render_state_from_parts(player_model=snapshot.player_model, motion=motion, block_registry=block_registry, arm_rotation_limit_min_deg=-180.0, arm_rotation_limit_max_deg=180.0, skin_texture_key=skin_texture_key))
  return tuple(states)
