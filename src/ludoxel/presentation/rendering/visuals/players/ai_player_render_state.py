...
def compose_ai_player_render_states(snapshots: tuple[AiPlayerRenderSnapshotDTO, ...], *, block_registry: BlockRegistry) -> tuple[PlayerRenderState, ...]:
  ...
  for snapshot in snapshots:
    ...
    states.append(
      compose_player_render_state_from_parts(player_model=snapshot.player_model, motion=motion, block_registry=block_registry, arm_rotation_limit_min_deg=-180.0, arm_rotation_limit_max_deg=180.0)
    )
  return tuple(states)
