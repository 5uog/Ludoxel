...
from ludoxel.presentation.rendering.visuals.players.skin_uv_maps import VISUAL_LEFT_ARM_BASE_UV_PX, VISUAL_LEFT_ARM_SLEEVE_UV_PX, VISUAL_RIGHT_ARM_BASE_UV_PX, VISUAL_RIGHT_ARM_SLEEVE_UV_PX, skin_cube_uv_map, uv_map_with_rotated_faces
...
def _build_player_model_pose_cached(state: PlayerRenderState | None) -> PlayerModelPose:
  ...
  if state is None:
    return PlayerModelPose(skin_face_rows=empty_faces, held_block_pose=None, special_item_face_rows=empty_faces, visible_special_item_icon=None, hurt_tint_strength=0.0, skin_texture_key=None, shadow_rows=empty_shadow)
  ...
  if bool(state.is_first_person):
    return PlayerModelPose(skin_face_rows=empty_faces, held_block_pose=None, special_item_face_rows=empty_faces, visible_special_item_icon=None, hurt_tint_strength=float(state.hurt_tint_strength), skin_texture_key=None if state.skin_texture_key is None else str(state.skin_texture_key), shadow_rows=shadow_rows)
