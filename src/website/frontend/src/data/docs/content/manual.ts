    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          {
            kind: 'paragraph',
            text: 'The square settings button on the bottom bar opens the embedded Chat Settings surface inside the chat screen, with the chat screen still visible behind it. Its only control in this version is Mute All Chat, a setting held only while the game runs. When the message field begins with a slash, the chat display hides the message list and shows a command candidate popup aligned to the lower edge of that display area. `/t`, `/teleport`, and `/tp` show the teleport signatures, while `/g` shows the game-mode signatures. Showing a candidate does not execute a command. Up and Down move the candidate selection while the popup is open; Tab inserts the selected command candidate, Esc closes the popup, and Enter still submits the current command text. With no popup open, Up and Down restore the running-session sent-input sequence, including submitted messages and commands. The full command behavior, the runtime-only mute, and the runtime history are described in the related articles.',
          },
          {
            kind: 'paragraph',
