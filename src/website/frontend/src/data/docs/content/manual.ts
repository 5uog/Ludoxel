    ...
    sections: [
      {
        ...
        body: [
          ...
          '`src/ludoxel/__main__.py` supplies process ingress, while `src/ludoxel/application/bootstrap/run.py` supplies the composition decision and `src/ludoxel/presentation/interface/windows/main.py` supplies the Qt shell. The bootstrap passes resolved project, resource, and runtime-data roots across that handoff; `MainWindow` and `GameScreen` use those values when the viewport begins loading its session and renderer state. The window becoming visible records successful arrival at the presentation shell. Persistence restoration, active-space selection, and frame-by-frame simulation remain later gates with their own state owners and failure paths.',
  ...
  if candidate is None:
    return`,
      ...
      },
      {
        ...
        body: [
          'The runtime data root is selected before any user-facing interaction; root selection fixes only the base location from which later state, cache, integrity, and store paths can be derived. Confirmation that a player name, inventory, My World payload, Othello payload, runtime preference, or saved world fragment has been loaded or accepted requires evidence beyond a selected `data_root`.',
  ...
  def _handle_loading_state_changed(self, active: bool) -> None:
    self._loading_overlay.setVisible(bool(active))`,
      ...
      },
      {
        ...
        body: [
          ...
          '`GLViewportWidget` accepts key, wheel, mouse-press, mouse-release, and mouse-move events during ordinary operation, then ignores each of those paths while `loading_active()` is true or the startup menu is open. Loading and the startup menu retain the preparation path while excluding gameplay mutation from these event handlers, so the menu receives input through its own widgets with the viewport event paths gated. The source fixes the in-application gate; compositor scheduling, driver behavior, and platform suspension remain outside this code path.',
        ],
        codeBlocks: [
          {
            language: 'py',
    ...
    self._set_runtime_active(bool(self._gl_initialized) and bool(self.isVisible()) and (bool(self._application_active) or bool(self.loading_active())) and (not bool(self._shutdown_done)))`,
      ...
      },
      {
        ...
        body: [
          'After the viewport finishes loading, the startup menu is the first interaction surface, and gameplay input and mouse capture stay behind a menu modal until a play button is pressed. My World then uses movement, camera control, hotbar state, inventory, block interaction, and world persistence. Othello uses board state, legal-move selection, side state, settings, board animation, and engine response. A missing menu after loading, or a play space entered without the menu, identifies a menu-admission or restored-state condition; module-entry evidence remains in the launch chain.',
          [
    ...
    sections: [
      {
        ...
        content: [
          ...
          {
            kind: 'paragraph',
            text: 'The menu carries the Ludoxel logo, a creator label fixed to the lower left, the runtime version label fixed to the lower right, a notification button placed in the lower-left side zone, and a player profile panel placed in the lower-right side zone. `StartupMenuPage` computes the Play My World, Play Othello (Reversi), and Quit panel from the page width alone, so the central button box remains centered at the minimum 1040 by 700 desktop window size while the side controls use the horizontal regions outside that central box. Quit closes the main window, which runs the application shutdown and final save, so it is the application exit control; the in-game pause Save & Quit instead saves the active play-space through the shared exit boundary and returns to this menu without closing the window, and it stays in the game and reports the error if that save fails. The menu does not start gameplay by itself; each button routes to a distinct surface.',
          },
        ],
      },
      {
        ...
        content: [
          {
            kind: 'paragraph',
            text: 'The menu profile panel reuses the same skin preview widget as the pause overlay and offers Change Skin and Reset to Timo with compact vertical action spacing. These actions call the same skin controller used by the pause overlay, so the menu and the in-game pause surface read and write one player skin state, and a change made on the menu is persisted through the existing preference path. The whole menu page forwards pointer motion, presses, and releases to the preview, so the model reacts to the cursor across the page as it does on the pause overlay while the menu buttons keep handling their own clicks. Dragging the preview rotates the model, the preview frame is rendered each frame while the menu page is visible, and the profile panel is assigned its fixed-size geometry directly in the right-side lower region instead of participating in a vertical root layout that can push the lower labels upward or reduce the 300 by 360 pixel preview box.',
          },
        ],
      },
      {
    ...
    if key == int(Qt.Key.Key_Escape):
      ...
      },
      {
        ...
        body: [
          ...
          '`_build_top_area` builds a fixed-width upper row as wide as the storage grid and centers it above the storage, so the black-background player preview on its left shares the left edge of the 9x4 storage. The preview box spans the first three storage columns and the two gaps between them, and its top sits one panel margin below the box top. `_TOP_AREA_TO_UPPER_GAP` combines the fixed non-overlap clearance with one `_SLOT_GAP`, so the preview and crafting row stop above the 9x3 storage without moving the thirty-six storage slots. The 30x30 `closeBtn` draws `assets/ui/inventory/close.svg` at the top-right of that row with its right edge on the storage right edge, and the 2x2 crafting input, the arrow, and the read-only 1x1 output sit centered between the preview box right edge and the storage right edge and below the close button, so only the crafting cluster yields to the close-button corner. The crafting output is read-only and resolves to empty because Ludoxel implements no crafting recipe resolution.',
      ...
      },
      {
        ...
        body: [
          ...
          'Search splits the query into tokens and keeps entries whose search key contains every token. `_apply_filter` lays the matches out from the top-left in row-major order across seven columns, the same order the full catalog uses, so a filtered result fills each row left to right before wrapping. The All Items box scrolls vertically only; its fixed width holds exactly the seven catalog columns beside the vertical scrollbar, and `_apply_filter` first hides every catalog button so the entries that fail the filter, and the whole catalog in Survival Mode, leave no stray slot behind.',
        ],
        codeBlocks: [
          {
            language: 'py',
    ...
    self._message.setText(body)`,
      ...
      },
      {
        ...
        body: [
          ...
          '`SessionManager.respawn` restores position from `SessionSettings.spawn_x`, `spawn_y`, and `spawn_z`, clears velocity, pitch, flight, eye offsets, queued jump and auto-jump state, overlap exemptions, health, death reason, and void timer. Those spawn settings follow the loaded world: `_sync_my_world_spawn_settings` in `src/ludoxel/application/persistence/schedulers/state.py` rewrites them from the loaded world’s generation spec through `spawn_for_generation` whenever a world is loaded into the session, so a flat world respawns the player one block above its ground layer and a seeded normal world respawns on its searched spawn column. `overlay_navigation.respawn` then hides the dead overlay and synchronizes hotbar widgets after it clears renderer selection. The active session player changes immediately; a later save remains a separate persistence operation.',
        ],
        codeBlocks: [
          {
            language: 'py',
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          {
            kind: 'paragraph',
            text: 'The square settings button on the bottom bar opens the embedded Chat Settings surface inside the chat screen, with the chat screen still visible behind it. Its only control in this version is Mute All Chat, a setting held only while the game runs. When user-edited text in the message field begins with a slash, the chat display hides the message list and shows a command candidate popup aligned to the lower edge of that display area. `/t`, `/teleport`, and `/tp` show the teleport signatures, while `/g` shows the game-mode signatures. Showing a candidate does not execute a command. Up and Down move the candidate selection while that popup is open from user-edited text; Tab inserts the selected command candidate, Esc closes the popup, and Enter still submits the current command text. When Up restores a sent-history row, even a row beginning with `/`, the field stays in recall state until the text is changed. During that clean recall state, Up and Down continue through the running-session sent-input sequence, including submitted messages, valid commands, and validation-error commands. Typing, deletion, paste, or selection replacement changes the recalled text and returns slash-command or mention candidates to the edited field. The full command behavior, the runtime-only mute, and the runtime history are described in the related articles.',
          },
          {
            kind: 'paragraph',
            text: 'When the current token starts with `@`, the same candidate popup lists the local player and current live AI names that match the typed prefix. Enter or Tab confirms the selected mention before message submission and replaces only that token with `@Name`; defeated or removed AI names are not kept as mention candidates. If a displayed message mentions the local player with an exact known `@Name` span, only that span is drawn in the yellow `§e` foreground. The raw message text does not need, store, or show a typed `§e` control sequence for the mention color.',
          },
        ],
      },
    ],
