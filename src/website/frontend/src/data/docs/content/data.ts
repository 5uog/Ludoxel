  ...
  raise LdxworldError("World package is not a Ludoxel world.")`,
      ...
      },
      {
        ...
        content: [
          {
            kind: 'paragraph',
            text: 'The active-world identifier in the index names the world whose body is held in the running My World session. `AppStateStore` reads the active entry into the in-memory `AppState` at load and writes the My World session back into the active entry at save through `save_space`, which also refreshes the entry game mode and updated timestamp. Opening a different world saves the outgoing world into its entry, passes the cached thumbnail bytes when present, clears the runtime thumbnail cache for the selected world, sets the new active identifier, and reloads the session from the new entry; deleting the loaded world clears the cache and rebinds the session to the new active world. The save path therefore writes to the world that is loaded, not to an arbitrary entry.',
          },
          {
            kind: 'paragraph',
