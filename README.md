...
本リポジトリには、本アプリケーションの独自素材、第三者素材、来歴確認を要する素材、実行時利用者データ、通常のアプリケーション出力、配布時資料、及びリポジトリ方針文書が混在し得る。これらは、技術的に同じリポジトリ内に存在し、同じ配布物に含まれ、又は同じ画面上に表示され得る場合であっても、法務上は同一の素材区分として扱われない。
...
本書にいう本アプリケーションの独自素材は、[`LICENSE`](LICENSE) が定義する `Original Materials` に対応する素材をいう。正確な定義、範囲、例外、及び第三者素材との区別は、[`LICENSE`](LICENSE) の英文本文に従う。
...
本アプリケーションの独自素材は、オープンソースソフトウェア又は自由ソフトウェアとして提供されるものではない。公開リポジトリとして閲覧できること、ホスティングサービス上で操作可能であること、又はローカル環境で技術的に取得若しくは実行できることは、[`LICENSE`](LICENSE) に明示された範囲を超える利用許諾を成立させない。

[`LICENSE`](LICENSE) に基づく通常のアプリケーション利用は、本アプリケーションを通常のデスクトップアプリケーションとして利用する限度で認められる。これにより、本アプリケーションの独自素材の複製、改変、翻案、再配布、転載、公衆送信、組込み、派生物作成、再ライセンス、販売、貸与、公開、配置、人工知能学習用データ化、データセット化、ベンチマーク用データ化、検索索引用データ化、又は自作発言が許諾されるものではない。

[`LICENSE`](LICENSE) に明示されていない利用は、小西拳斗が署名した別個の書面によって許諾された場合を除き、許されない。ホスティングサービスの画面機能、リポジトリの公開状態、補助文書の存在、第三者素材のライセンス表示の存在、又は第三者素材のライセンス本文の存在を、当該許諾に代えることはできない。
...
CPython、PyQt6、Qt 実行環境構成要素、NumPy、PyOpenGL、wgpu、rendercanvas、PyInstaller、Ruff、Node.js、npm パッケージ、ESLint、Stylelint、Prettier、及び関連する実行環境又はツールチェーンは、それぞれの上流ライセンス条件に従う。当該依存物の正確なライセンス条件、版、同梱可否、及び再配布条件は、配布前の確認事項として扱う。
