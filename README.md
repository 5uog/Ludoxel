...
Within this space, block placement, block breaking, and the block inventory overlay are disabled. The Othello hotbar is reserved for control items. Slot `1` contains `Start`, and slot `9` contains `Settings`. Right-clicking `Start` begins a fresh match or restarts the current match under the stored defaults. Right-clicking `Settings` opens the Othello settings dialog, which configures AI strength, time control, and player order for the next match. The difficulty menu now exposes `Weak`, `Medium`, `Strong`, and `Insane`. `Insane` uses a bundled opening-book file together with a cached bitboard search that reuses transposition state for the lifetime of a match generation inside the AI worker. The default Othello configuration remains `Medium`, `20 minutes per side`, and `player moves first`.
