...
At startup, the shell now presents a dedicated top-level loading screen before the main window is allowed to surface, and the embedded viewport still retains its in-window loading overlay until renderer initialization has completed and the initially visible chunk set has reached GPU residency. Simulation input remains suspended during that phase, so the user does not observe the previous in-world hitching while the initial chunk uploads are still converging and the launch path no longer leaks an initial blank frame from the not-yet-initialized OpenGL surface. Space transfers between `My World` and `Play Othello (Reversi)` reuse a retained CPU-side chunk-build cache, immediately re-enter the same loading overlay while the target residency set is unresolved, and prewarm the inactive play space after a load completes so that the next transfer does not pay the same rebuild cost again. While the desktop shell is inactive, the fixed-step runner and render timers are explicitly suspended and then re-armed on return, which removes the previous multi-second catch-up stutter that followed a task switch back into the application.
