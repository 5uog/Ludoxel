...
// Deterministic terrain mathematics. Every formula, constant, salt, and
...
const SALT_HEIGHT_BASE: u64 = 1;
const SALT_RAVINE: u64 = 101;
const SALT_RAVINE_MASK: u64 = 102;
const SALT_RAVINE_DEPTH: u64 = 103;
const SALT_DEEPSLATE: u64 = 120;
const SALT_DIRT_DEPTH: u64 = 130;
const SALT_DIRT_MIX: u64 = 131;
const SALT_DIRT_PATCH: u64 = 132;
const SALT_STONE_A: u64 = 140;
const SALT_STONE_B: u64 = 141;
const SALT_ORE_GATE: u64 = 150;
const SALT_EMERALD: u64 = 151;
const SALT_ORE_COAL: u64 = 160;
const SALT_ORE_COPPER: u64 = 161;
const SALT_ORE_IRON: u64 = 162;
const SALT_ORE_GOLD: u64 = 163;
const SALT_ORE_REDSTONE: u64 = 164;
const SALT_ORE_LAPIS: u64 = 165;
const SALT_ORE_DIAMOND: u64 = 166;
...
#[inline]
fn hash_u64(seed: i64, salt: u64, a: i64, b: i64, c: i64) -> u64 {
  let mut v = seed as u64;
  v ^= salt.wrapping_mul(0x9E3779B97F4A7C15);
...
#[inline]
fn lattice_value(seed: i64, salt: u64, a: i64, b: i64, c: i64) -> f64 {
  let h = hash_u64(seed, salt, a, b, c);
...
fn value_noise_2d(seed: i64, salt: u64, x: f64, z: f64, scale: f64) -> f64 {
  let fx = x / scale;
  let fz = z / scale;
  let x0 = fx.floor();
  let z0 = fz.floor();
  ...
  let xi = x0 as i64;
  let zi = z0 as i64;
  let n00 = lattice_value(seed, salt, xi, zi, 0);
  let n10 = lattice_value(seed, salt, xi + 1, zi, 0);
  let n01 = lattice_value(seed, salt, xi, zi + 1, 0);
  let n11 = lattice_value(seed, salt, xi + 1, zi + 1, 0);
  let nx0 = n00 + (n10 - n00) * sx;
  let nx1 = n01 + (n11 - n01) * sx;
  nx0 + (nx1 - nx0) * sz
}

fn value_noise_3d(seed: i64, salt: u64, x: f64, y: f64, z: f64, scale: f64) -> f64 {
  let fx = x / scale;
  let fy = y / scale;
  let fz = z / scale;
  let x0 = fx.floor();
  ...
  let yi = y0 as i64;
  let zi = z0 as i64;
  let n000 = lattice_value(seed, salt, xi, zi, yi);
  let n100 = lattice_value(seed, salt, xi + 1, zi, yi);
  let n010 = lattice_value(seed, salt, xi, zi + 1, yi);
  let n110 = lattice_value(seed, salt, xi + 1, zi + 1, yi);
  let n001 = lattice_value(seed, salt, xi, zi, yi + 1);
  let n101 = lattice_value(seed, salt, xi + 1, zi, yi + 1);
  let n011 = lattice_value(seed, salt, xi, zi + 1, yi + 1);
  let n111 = lattice_value(seed, salt, xi + 1, zi + 1, yi + 1);
...
fn raw_surface_height(seed: i64, x: i64, z: i64) -> f64 {
  ...
  let mut salt = SALT_HEIGHT_BASE;
  for (amplitude, wavelength) in HEIGHT_OCTAVES.iter() {
    total += amplitude * value_noise_2d(seed, salt, x as f64, z as f64, *wavelength);
    salt += 1;
  }
  total
}
...
fn ravine_depth(seed: i64, x: i64, z: i64) -> f64 {
  let mask = value_noise_2d(seed, SALT_RAVINE_MASK, x as f64, z as f64, RAVINE_MASK_SCALE);
  if mask <= RAVINE_MASK_MIN {
    return 0.0;
  }
  let ridge = value_noise_2d(seed, SALT_RAVINE, x as f64, z as f64, RAVINE_SCALE);
  ...
  let s = smoothstep(t);
  let mut depth_amp = 14.0 + 7.0 * value_noise_2d(seed, SALT_RAVINE_DEPTH, x as f64, z as f64, RAVINE_DEPTH_SCALE);
  if depth_amp < 4.0 {
    depth_amp = 4.0;
  }
  s * depth_amp
...
fn stone_material(seed: i64, x: i64, y: i64, z: i64, deepslate: bool) -> u8 {
  ...
  let v1 = value_noise_3d(seed, SALT_STONE_A, x as f64, y as f64, z as f64, 34.0);
  if v1 > 0.52 {
  ...
  let v2 = value_noise_3d(seed, SALT_STONE_B, x as f64, y as f64, z as f64, 23.0);
  if v2 > 0.55 {
...
fn ore_material(seed: i64, x: i64, y: i64, z: i64, deepslate: bool) -> u8 {
  let gate = hash_u64(seed, SALT_ORE_GATE, x, z, y) % 1000;
  let mut ore: u8 = 0;
  if gate < 350 {
    if y <= -40 && value_noise_3d(seed, SALT_ORE_DIAMOND, x as f64, y as f64, z as f64, 10.0) > 0.78 {
      ore = MATERIAL_DIAMOND_ORE;
    } else if (-64..=-8).contains(&y) && value_noise_3d(seed, SALT_ORE_GOLD, x as f64, y as f64, z as f64, 12.0) > 0.72 {
      ore = MATERIAL_GOLD_ORE;
    } else if (-64..=-16).contains(&y) && value_noise_3d(seed, SALT_ORE_REDSTONE, x as f64, y as f64, z as f64, 12.0) > 0.72 {
      ore = MATERIAL_REDSTONE_ORE;
    } else if (-64..=-16).contains(&y) && value_noise_3d(seed, SALT_ORE_LAPIS, x as f64, y as f64, z as f64, 12.0) > 0.74 {
      ore = MATERIAL_LAPIS_ORE;
    } else if y <= 20 && value_noise_3d(seed, SALT_ORE_IRON, x as f64, y as f64, z as f64, 14.0) > 0.66 {
      ore = MATERIAL_IRON_ORE;
    } else if y >= -16 && value_noise_3d(seed, SALT_ORE_COPPER, x as f64, y as f64, z as f64, 14.0) > 0.66 {
      ore = MATERIAL_COPPER_ORE;
    } else if value_noise_3d(seed, SALT_ORE_COAL, x as f64, y as f64, z as f64, 16.0) > 0.62 {
      ore = MATERIAL_COAL_ORE;
    }
  }
  if ore == 0 && y >= 0 && hash_u64(seed, SALT_EMERALD, x, z, y) % 4000 == 0 {
...
pub fn material_code(seed: i64, _version: u32, mode: u32, flat_ground_y: i32, x: i64, y: i64, z: i64, column_height: i64) -> u8 {
  ...
  let deepslate_boundary = -28.0 + 5.0 * value_noise_2d(seed, SALT_DEEPSLATE, x as f64, z as f64, 60.0);
  ...
  if !in_ravine {
    let dirt_depth = 2 + (hash_u64(seed, SALT_DIRT_DEPTH, x, z, 0) % 2) as i64;
    if depth <= dirt_depth {
      let patch = value_noise_2d(seed, SALT_DIRT_PATCH, x as f64, z as f64, 24.0);
      let roll = (hash_u64(seed, SALT_DIRT_MIX, x, z, y) % 100) as i64;
      if patch > 0.45 {
